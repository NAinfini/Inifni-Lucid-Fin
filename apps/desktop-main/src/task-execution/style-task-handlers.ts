...
import type { TaskExecutionContext, TaskHandler } from '@lucid-fin/application';
import type {
  ColorStyle,
  ColorSwatch,
  ExposureProfile,
...
import { parseTaskListId } from '@lucid-fin/contracts-parse';
...
export function createStyleTaskHandlers(options: StyleTaskHandlerOptions): TaskHandler[] {
  ...
  return [
    {
      ...
      async execute(context) {
        const assetHash = requireString(context.task.input.assetHash, 'assetHash');
        const assetType = requireAssetType(context.task.input.assetType);
        const asset = context.db.repos.assets.findByHash(assetHash);
    ...
    },
    {
      ...
      async execute(context) {
        ...
        context.db.repos.taskLists.updateTaskList(parseTaskListId(context.taskList.id), {
          output: {
            ...context.taskList.output,
...
function getTaskOutput(context: TaskExecutionContext, taskId: string): Record<string, unknown> {
  const task = context.db.repos.taskLists
    .listTasks(parseTaskListId(context.taskList.id))
    .rows.find((entry) => entry.taskKey === taskId);

  if (!task) {
    throw new Error(`Task "${taskId}" not found in Task List ${context.taskList.id}`);
  }
