...
import { createStyleTaskHandlers } from './style-task-handlers.js';

function tmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'lucid-style-task-list-'));
}
...
describe('createStyleTaskHandlers', () => {
  let base: string;
  let db: SqliteIndex;
  ...
  it('executes style.extract end-to-end and persists a generated color style artifact', async () => {
    ...
    const taskExecutionEngine = new TaskExecutionEngine({
      ...
      handlers: createStyleTaskHandlers({
        cas: {
          getAssetPath: () => assetPath,
        } as Parameters<typeof createStyleTaskHandlers>[0]['cas'],
        llmRegistry,
        now: () => 1000,
        idFactory: nextId,
      }),
    ...
    const taskList = taskExecutionEngine.get(taskListId);
    const persistArtifacts = db.repos.taskLists.listArtifactsByTask('task-persist' as never);
  ...
  it('extracts a video frame before calling the LLM for video assets', async () => {
    ...
    const _taskListId = taskExecutionEngine.start({
      taskListType: 'style.extract',
      entityType: 'asset',
      entityId: 'video-hash',
      input: {
