...
import type { DataAccessOptions } from '@lucid-fin/storage';
import {
  createJourneyDependencies,
  createJourneyPrivateRecoveryCodec,
  deterministicIds,
} from '../../../packages/storage/test/i2h/fixture.js';
import {
  createElectronPushSink,
  startElectronHost,
...
async function dataFixture(): Promise<{
  ...
  const directory = await mkdtemp(join(tmpdir(), 'lucid-fin-electron-host-'));
...
function windowFixture(order: string[]) {
  ...
  const send = vi.fn();
  const window: RendererWindowLike = {
    webContents: {
...
describe('Electron host', () => {
  it('creates and loads the renderer only after the complete desktop startup barrier', async () => {
    ...
    const host = await startElectronHost({
      ...
      createWindow: (preloadPath) => {
        ...
        expect(preloadPath).toMatch(/preload\.cjs$/u);
  ...
  it('sends pushes only to a live renderer webContents', () => {
    const order: string[] = [];
    const renderer = windowFixture(order);
    let window: RendererWindowLike | null = null;
    const sink = createElectronPushSink(() => window);
    const push = { kind: 'push' } as WirePushV1;

    sink.send(push);
    window = renderer.window;
    sink.send(push);
    expect(renderer.send).toHaveBeenCalledWith(LUCID_FIN_WIRE_PUSH_CHANNEL_V1, push);
    renderer.window.destroy();
    sink.send(push);
    expect(renderer.send).toHaveBeenCalledOnce();
  });

  it('destroys a half-loaded renderer and closes desktop authorities when loading fails', async () => {
    const setup = await dataFixture();
    const order: string[] = [];
    const ipcMain = ipcFixture(order);
    const mediaPreviewProtocol = mediaPreviewProtocolFixture(order);
