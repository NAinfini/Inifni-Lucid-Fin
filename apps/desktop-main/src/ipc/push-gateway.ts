...
import type { RunsAuthority } from '@lucid-fin/storage';

export interface WirePushSink {
  send(push: WirePushV1): void;
}

export interface RunEventPushGateway {
  publishPersisted(runId: string, cursor: RunEventCursorV1): WirePushV1;
}

export interface RunEventPushGatewayOptions {
  readonly createRequestId: () => string;
}

export interface PersistedRunEventPublisher {
  publishHead(run: Pick<Run, 'id' | 'publicEventHead'>): void;
}

export interface PersistedRunEventPublisherOptions extends RunEventPushGatewayOptions {
...
export function createRunEventPushGateway(
  runs: Pick<RunsAuthority, 'listPublicEvents'>,
  sink: WirePushSink,
  options: RunEventPushGatewayOptions,
): RunEventPushGateway {
...
export function createPersistedRunEventPublisher(
  runs: Pick<RunsAuthority, 'listPublicEvents'>,
  sink: WirePushSink,
  options: PersistedRunEventPublisherOptions,
): PersistedRunEventPublisher {
