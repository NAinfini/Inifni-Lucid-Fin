import type { Run, WireRequestV1, WireSuccessV1 } from '@lucid-fin/contracts';
import type { MessageSendAcceptanceSeed, CommandContext, DataAccess } from '@lucid-fin/storage';
import type { HostConfirmationAuthority, HostInteractionAuthority } from '@lucid-fin/storage/host';
import type { MediaPreviewCapabilityGateway } from '../media-preview.js';
import type { WireHandler, WireHandlers } from './router.js';

type SeededRunRequest = Extract<
  WireRequestV1,
  { readonly method: 'message.send' | 'run.sendFollowup' }
>;
export interface WireUseCaseDependencies {
  ...
  readonly pickExportDestination: WireHandler<'os.export.pick'>;
...
export function createWireUseCaseHandlers(dependencies: WireUseCaseDependencies): WireHandlers {
    ...
    'decision.record': (request, context) =>
      request.input.action === 'undo'
        ? data.userChoices.undoChoice(
            request as Parameters<DataAccess['userChoices']['undoChoice']>[0],
            context,
          )
        : data.userChoices.recordResultDecision(
            request as Parameters<DataAccess['userChoices']['recordResultDecision']>[0],
            context,
          ),
