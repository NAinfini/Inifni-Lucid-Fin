...
import type { PublicRunEvent, Run, WireSuccessV1 } from '@lucid-fin/contracts';
import { createPersistedRunEventPublisher, createRunEventPushGateway } from './push-gateway.js';
...
describe('Run event push gateway', () => {
  it('re-reads the exact persisted event before broadcasting it', () => {
    ...
    const gateway = createRunEventPushGateway(
      { listPublicEvents },
      { send },
  ...
  it('does not broadcast a missing or hash-mismatched persisted event', () => {
    const send = vi.fn();
    const missing = createRunEventPushGateway(
    ...
    const mismatched = createRunEventPushGateway(
  ...
  it('pushes every missing public event through a batched Run head in persisted order', () => {
    ...
    const send = vi.fn();
    const onError = vi.fn();
    const publisher = createPersistedRunEventPublisher(
      { listPublicEvents },
      { send },
