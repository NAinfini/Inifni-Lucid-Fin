...
import { parseCharacterId, parseEquipmentId, parseLocationId, parseAssetHash } from '@lucid-fin/contracts-parse';
...
export function registerFolderHandlers(ipcMain: IpcMain, db: SqliteIndex): void {
  ...
  for (const kind of KINDS) {
    ...
    ipcMain.handle(
      `folder.${kind}:rename`,
      async (_e, args: { id: string; name: string }) => {
  ...
  ipcMain.handle(
    'asset:setFolder',
    async (_e, args: { hash: string; folderId: string | null }) => {
