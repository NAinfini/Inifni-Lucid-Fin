...
describe('registerExportHandlers', () => {
  ...
  it('exports NLE timelines to the requested output path and logs file size', async () => {
    ...
    const result = await exportNle?.(
      {},
      {
        ...
        outputPath: path.join(APP_ROOT, 'timeline.fcpxml'),
      },
    );
    ...
    expect(fsp.writeFile).toHaveBeenCalledWith(
      path.join(APP_ROOT, 'timeline.fcpxml'),
      '<fcpxml />',
      'utf8',
    );
  ...
  it('cancels NLE export through the save dialog and validates bad NLE requests', async () => {
    ...
    await expect(
      exportNle?.({}, { format: 'bad', project: { metadata: { title: 'Pilot' }, tracks: [] } }),
    ).rejects.toThrow('export:nle: format must be "fcpxml" or "edl"');
    await expect(exportNle?.({}, { format: 'edl' })).rejects.toThrow(
      'export:nle: project is required',
    );
  ...
  it('exports asset bundles as zip archives using resolved CAS file paths', async () => {
    ...
    const result = await exportBundle?.(
      {},
      {
        ...
        outputPath: path.join(APP_ROOT, 'assets.zip'),
      },
    );
  ...
  it('rejects asset bundle export when CAS is unavailable, hashes are missing, or no files resolve', async () => {
    ...
    await expect(
      exportBundle?.(
        {},
        { assetHashes: ['missing'], outputPath: path.join(APP_ROOT, 'assets.zip') },
      ),
  ...
  it('exports subtitles in srt and ass formats and validates subtitle requests', async () => {
    ...
    await expect(
      exportSubtitles?.(
        {},
        {
          format: 'srt',
          cues,
          outputPath: path.join(APP_ROOT, 'captions.srt'),
        },
      ),
    ...
    await expect(
      exportSubtitles?.(
        {},
        {
          format: 'ass',
          cues,
          outputPath: path.join(APP_ROOT, 'captions.ass'),
          videoWidth: 1920,
          videoHeight: 1080,
        },
      ),
    ...
    await expect(
      exportSubtitles?.(
        {},
        { format: 'bad', cues, outputPath: path.join(APP_ROOT, 'captions.txt') },
      ),
  ...
  it('opens save dialog when subtitles outputPath is omitted and cancels gracefully', async () => {
    ...
    await expect(exportSubtitles?.({}, { format: 'srt', cues })).resolves.toBeNull();
  ...
  it('rejects subtitles export with path traversal or paths outside allowed roots', async () => {
    resetCommon();
    const handlers = registerHandlers();
    const exportSubtitles = handlers.get('export:subtitles');
    const cues = [{ id: 'cue-1', startTime: 0, endTime: 1, text: 'hello' }];
  ...
  it('exports storyboard PDFs with resolved thumbnails and supports dialog cancellation', async () => {
    ...
    vi.spyOn(fs, 'existsSync').mockImplementation((target) =>
      String(target).includes('frame-1.png'),
    );
    showSaveDialog.mockResolvedValueOnce({ canceled: true, filePath: undefined });
    const cas = {
      getAssetPath: vi.fn(
    ...
    const result = await exportStoryboard?.(
      {},
      {
        ...
        projectTitle: 'Pilot',
        outputPath: path.join(APP_ROOT, 'storyboard.pdf'),
      },
    );
  ...
  it('exports metadata as JSON and CSV with proper field escaping and dialog cancellation', async () => {
    ...
    await expect(
      exportMetadata?.(
        {},
        {
          ...
          nodes,
          projectTitle: 'Pilot',
          outputPath: path.join(APP_ROOT, 'metadata.json'),
        },
      ),
    ).resolves.toEqual({
      outputPath: path.join(APP_ROOT, 'metadata.json'),
    ...
    await expect(
      exportMetadata?.(
        {},
        {
          format: 'csv',
          nodes,
          outputPath: path.join(APP_ROOT, 'metadata.csv'),
        },
      ),
    ).resolves.toEqual({
      outputPath: path.join(APP_ROOT, 'metadata.csv'),
    ...
    expect(fsp.writeFile).toHaveBeenNthCalledWith(
      1,
      path.join(APP_ROOT, 'metadata.json'),
      expect.stringContaining('"project": "Pilot"'),
      'utf8',
    );
    expect(fsp.writeFile).toHaveBeenNthCalledWith(
      2,
      path.join(APP_ROOT, 'metadata.csv'),
