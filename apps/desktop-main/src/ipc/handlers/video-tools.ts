...
export function createVideoTools(deps: VideoToolDeps): AgentTool[] {
  const cloneTool: AgentTool = {
    name: 'video.clone',
    description:
      'Analyze a video file by detecting scene cuts, extracting keyframes, and auto-describing them. Creates a new canvas with one video node per scene, ready for re-generation.',
    tier: 2,
    parameters: {
      type: 'object',
      properties: {
        ...
        threshold: {
          type: 'number',
          description: 'Scene detection sensitivity (0-1, default 0.4)',
        },
      },
      required: ['filePath'],
    },
