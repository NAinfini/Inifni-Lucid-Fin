/**
 * Preset domain service — all preset CRUD, validation, import/export logic.
 *
 * Extracted from preset.handlers.ts. The handler file now only contains thin
 * IPC routing; all business logic lives here.
 */
import {
...
export function initPresetService(db: SqliteIndex): void {
  _db = db;
  globalPresetState = null; // reset to force re-hydration from new db
}

export function getPresetState(): ProjectPresetState {
...
export function ensurePresetDefinition(input: unknown, opName: string): PresetDefinition {
...
export function savePreset(state: ProjectPresetState, input: PresetDefinition): PresetDefinition {
...
export function deletePreset(state: ProjectPresetState, id: string): void {
...
export function ensureResetRequest(input: unknown): PresetResetRequest {
  if (!input || typeof input !== 'object') {
...
export function ensureImportPayload(input: unknown): PresetLibraryImportPayload {
  if (!input || typeof input !== 'object') {
...
export function ensureExportRequest(input: unknown): PresetLibraryExportRequest | undefined {
