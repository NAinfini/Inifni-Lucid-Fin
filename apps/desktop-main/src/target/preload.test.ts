...
describe('generated target preload', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  ...
  it('exposes exactly the 39 canonical use cases through one invoke channel', async () => {
    ...
    const api = exposeInMainWorld.mock.calls[0]?.[1] as TargetDesktopApiV1;
    ...
    expect(invoke).toHaveBeenCalledWith(TARGET_WIRE_INVOKE_CHANNEL_V1, {
      wireVersion: 1,
      kind: 'request',
    ...
    expect(new Set(invoke.mock.calls.map((call) => call[0]))).toEqual(
      new Set([TARGET_WIRE_INVOKE_CHANNEL_V1]),
    );
  });
  ...
  it('exposes one typed Run push subscription with exact listener cleanup', async () => {
    ...
    on.mockImplementation((channel: string, value: typeof listener) => {
      if (channel === TARGET_WIRE_PUSH_CHANNEL_V1) listener = value;
    });
    await import('./preload.generated.cjs');
    const api = exposeInMainWorld.mock.calls[0]?.[1] as TargetDesktopApiV1;
    ...
    dispose();
    expect(removeListener).toHaveBeenCalledWith(TARGET_WIRE_PUSH_CHANNEL_V1, listener);
  });
});
