...
import type { TargetCommandContext } from '@lucid-fin/target-storage';
import { TargetWirePublicError } from './ipc/router.js';
import {
  createTargetExportDestinationGateway,
  type TargetExportDestinationPickerResult,
...
describe('target export destination gateway', () => {
  it('fails closed without a main-owned picker adapter', async () => {
    const gateway = createTargetExportDestinationGateway();

    await expect(gateway.pick(request, context)).rejects.toEqual(
      expect.objectContaining<TargetWirePublicError>({
  ...
  it('fails closed when the gateway closes while the picker is pending', async () => {
    let settlePicker!: (value: TargetExportDestinationPickerResult) => void;
    const selection = new Promise<TargetExportDestinationPickerResult>((resolve) => {
