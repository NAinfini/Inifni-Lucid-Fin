...
export const TARGET_MEDIA_PREVIEW_PROTOCOL_SCHEME = 'lucid-target-media' as const;
const PREVIEW_URL_HOST = 'preview';
const DEFAULT_CAPABILITY_LIFETIME_MS = 5 * 60 * 1_000;
const MAX_CAPABILITIES = 1_000;

export interface TargetMediaPreviewProtocolRequest {
  readonly url: string;
  readonly method: string;
  readonly headers: Pick<Headers, 'get'>;
}

export interface TargetMediaPreviewProtocol {
  handle(
    scheme: string,
...
export interface TargetMediaPreviewSchemeRegistrar {
...
export const TARGET_MEDIA_PREVIEW_SCHEME_REGISTRATION = Object.freeze([
  Object.freeze({
    scheme: TARGET_MEDIA_PREVIEW_PROTOCOL_SCHEME,
...
export function registerTargetMediaPreviewScheme(
  registrar: TargetMediaPreviewSchemeRegistrar,
): void {
  registrar.registerSchemesAsPrivileged(TARGET_MEDIA_PREVIEW_SCHEME_REGISTRATION);
}
...
export interface TargetMediaPreviewCapabilityGateway {
  issue(input: MediaPreviewIssueInputV1): MediaPreviewCapabilityGrantV1;
  respond(request: TargetMediaPreviewProtocolRequest): Promise<Response>;
  close(): void;
}

export interface TargetMediaPreviewCapabilityGatewayOptions {
  readonly sourceResolver: TargetMediaPreviewSourceResolver;
  readonly now?: () => Date;
...
function tokenFromPreviewUrl(value: string): string | null {
  try {
    ...
    if (
      url.protocol !== `${TARGET_MEDIA_PREVIEW_PROTOCOL_SCHEME}:` ||
...
function readHeader(request: TargetMediaPreviewProtocolRequest, name: string): string | null {
...
function responseHeaders(
  source: TargetMediaPreviewSource,
  range: RangeParse,
): Record<string, string> {
  const mime = /^(image|video|audio)\/[a-z0-9!#$&^_.+-]+$/u.exec(source.mimeType);
  if (mime === null || mime[1] !== source.kind) {
    throw new TargetStorageError('CORRUPT_DATA', 'Media preview MIME type is invalid');
  }
...
export function createTargetMediaPreviewCapabilityGateway(
  options: TargetMediaPreviewCapabilityGatewayOptions,
): TargetMediaPreviewCapabilityGateway {
  ...
  return Object.freeze({
    issue(inputValue: MediaPreviewIssueInputV1) {
      ...
      return parseCanonical(MediaPreviewCapabilityGrantV1Schema, {
        url: `${TARGET_MEDIA_PREVIEW_PROTOCOL_SCHEME}://${PREVIEW_URL_HOST}/${token}`,
    ...
    async respond(request: TargetMediaPreviewProtocolRequest) {
      ...
      let source: TargetMediaPreviewSource;
      try {
...
export function installTargetMediaPreviewProtocol(
  protocol: TargetMediaPreviewProtocol,
  gateway: TargetMediaPreviewCapabilityGateway,
): () => void {
  protocol.handle(TARGET_MEDIA_PREVIEW_PROTOCOL_SCHEME, (request) => gateway.respond(request));
  let installed = true;
  return () => {
    if (!installed) return;
    installed = false;
    protocol.unhandle(TARGET_MEDIA_PREVIEW_PROTOCOL_SCHEME);
  };
}
