...
import {
  TARGET_MEDIA_PREVIEW_PROTOCOL_SCHEME,
...
function source(
  ...
  return {
    ...
    verify,
    open,
  } satisfies TargetMediaPreviewSource;
}
...
): TargetMediaPreviewProtocolRequest {
  return {
    url,
    method,
...
describe('target media preview capabilities', () => {
  it('issues opaque URLs and serves verified GET, HEAD, and a single byte range', async () => {
    ...
    const gateway = createTargetMediaPreviewCapabilityGateway({
    ...
    expect(grant).toEqual({
      url: 'lucid-target-media://preview/cap_preview_capability_1234567890',
  ...
  it('rejects forged, expired, multi-range, and malformed capability requests without leaking state', async () => {
    ...
    expect(
      (await gateway.respond(request('lucid-target-media://preview/cap_forged_token_1234567890')))
        .status,
    ).toBe(404);
  ...
  it('returns a sanitized failure when access-time CAS integrity verification fails and clears on close', async () => {
    ...
    const verifyBeforeOpen = vi.fn(async () => {
      throw new TargetStorageError('CORRUPT_DATA', 'C:\\private\\cas\\secret.mp4');
    });
  ...
  it('registers the one privileged scheme and precisely removes only its installed handler', () => {
    ...
    registerTargetMediaPreviewScheme({ registerSchemesAsPrivileged });
    ...
    const dispose = installTargetMediaPreviewProtocol(protocol, gateway);
    ...
    expect(protocol.unhandle).toHaveBeenCalledWith(TARGET_MEDIA_PREVIEW_PROTOCOL_SCHEME);
  });
});
