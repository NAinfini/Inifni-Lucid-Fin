...
import { startDesktopComposition } from './composition-root.js';
...
async function fixture() {
  const directory = await mkdtemp(join(tmpdir(), 'lucid-fin-ipc-'));
  ...
  const dataAccess: Omit<DataAccessOptions, 'deliveryDestinationGrants'> = {
    now: () => NOW,
    createId,
...
function ipcFixture() {
  ...
  return {
    ipcMain: {
      handle: vi.fn(
        (channel: string, next: (event: object, input: unknown) => Promise<WireResponseV1>) => {
          expect(channel).toBe(LUCID_FIN_WIRE_INVOKE_CHANNEL_V1);
          listener = next;
        },
      ),
      removeHandler: vi.fn(() => {
    ...
    async invoke<Method extends PublicWireMethodV1>(
      ...
      if (listener === undefined) throw new Error('Desktop IPC listener is not ready');
...
describe('desktop single-router IPC journey', () => {
  it('persists the project-first path and queries all project workspaces through one channel', async () => {
    ...
    try {
      ...
      if (exportDestination.result.state !== 'selected') {
        throw new Error('Expected the desktop export picker to select a destination');
      }
      ...
      const reconnect = await startDesktopComposition({
        ...setup,
