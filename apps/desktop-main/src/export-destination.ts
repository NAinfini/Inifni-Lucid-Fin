...
export type ExportDestinationPickerResult =
...
export interface ExportDestinationPickerAdapter {
  pick(input: ExportPickerInput): Promise<ExportDestinationPickerResult>;
}

export interface ExportDestinationGatewayOptions {
  readonly picker?: ExportDestinationPickerAdapter;
  readonly now?: () => Date;
...
export interface ExportDestinationGateway extends DeliveryDestinationGrantResolver {
  readonly pick: WireHandler<'os.export.pick'>;
  close(): void;
}
...
function unavailable(): WirePublicError {
  return new WirePublicError({ code: 'unavailable', retryable: false });
}
...
export function createExportDestinationGateway(
  options: ExportDestinationGatewayOptions = {},
): ExportDestinationGateway {
  ...
  const pick: WireHandler<'os.export.pick'> = async (request) => {
  ...
  return Object.freeze({
    ...
    async resolve(
      ...
      if (grant === undefined)
        throw new StorageError('NOT_FOUND', 'Delivery export destination expired');
      if (
