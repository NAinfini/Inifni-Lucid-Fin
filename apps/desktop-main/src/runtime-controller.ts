import type { CanonicalModelRequestV1, Run } from '@lucid-fin/contracts';
import {
  coordinateRun,
  createStorageReadToolExecutor,
  drainRequestedOperationCancellations,
  recoverActivation,
  type RecoverActivationInput,
  type RunCoordinationResult,
  type RunCoordinatorDependencies,
  type ModelAdapter,
} from '@lucid-fin/runtime';
import type { HarnessActivationSnapshot, DataAccess } from '@lucid-fin/storage';
import type { DesktopRuntimeController } from './composition-root.js';

interface RuntimeControllerKernel {
  coordinate(
  ...
  recover(
    dependencies: { readonly persistence: DataAccess['harness'] },
    input: RecoverActivationInput,
  ): ReturnType<typeof recoverActivation>;
...
export interface RuntimeControllerOptions {
  ...
  readonly kernel?: RuntimeControllerKernel;
}

const defaultKernel: RuntimeControllerKernel = Object.freeze({
  coordinate: coordinateRun,
  recover: recoverActivation,
});

function commanderContext(runId: string, createId: RuntimeControllerOptions['createId']) {
  return {
...
function activeSnapshot(data: DataAccess, runId: string): HarnessActivationSnapshot | null {
...
function currentRun(data: DataAccess, runId: string, requestId: string): Run {
...
export function createRuntimeController(
  ...
  const dependencies: RunCoordinatorDependencies = {
    ...
    model: options.model,
    toolExecutor: createStorageReadToolExecutor(data),
