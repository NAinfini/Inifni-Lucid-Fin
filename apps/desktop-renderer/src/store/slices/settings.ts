...
export const settingsSlice = createSlice({
  ...
  reducers: {
    ...
    recordPrompt(state, action: PayloadAction<{ wordCount: number }>) {
      const today = new Date().toISOString().slice(0, 10);
    ...
    recordEntityCreate(state, action: PayloadAction<{ entityType: 'character' | 'location' | 'equipment' | 'prop' }>) {
      const today = new Date().toISOString().slice(0, 10);
    ...
    recordEntityEdit(state) {
      state.usage.entityEdits += 1;
    },
    recordPresetChange(state, action: PayloadAction<{ category: string }>) {
    ...
    recordFeatureUsed(state, action: PayloadAction<{ feature: string }>) {
    ...
    recordTokenUsage(state, action: PayloadAction<{ providerId: string; inputTokens: number; outputTokens: number }>) {
      const today = new Date().toISOString().slice(0, 10);
