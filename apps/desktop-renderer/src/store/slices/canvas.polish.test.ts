...
function setup() {
  let state = canvasSlice.reducer(
    undefined,
    setCanvases([makeCanvas(), makeCanvas('canvas-2', 'Alt')]),
  );
  ...
  state = canvasSlice.reducer(
    state,
    addNode({ id: 'image-1', type: 'image', position: { x: 180, y: 0 } }),
  );
  state = canvasSlice.reducer(
    state,
    addNode({ id: 'video-1', type: 'video', position: { x: 360, y: 0 } }),
  );
  return state;
}
...
describe('canvas polish reducers', () => {
  it('creates backdrop nodes with explicit size metadata', () => {
    ...
    const node = state.canvases.entities['canvas-1']!.nodes.find(
      (entry) => entry.id === 'backdrop-1',
    );
  ...
  it('auto-labels edges based on connected node types', () => {
    ...
    expect(
      state.canvases.entities['canvas-1']!.edges.find((edge) => edge.id === 'edge-1')?.data.label,
    ).toBe(t('edge.generateImage'));
    expect(
      state.canvases.entities['canvas-1']!.edges.find((edge) => edge.id === 'edge-2')?.data.label,
    ).toBe(t('edge.animate'));
  });
  ...
  it('copies selected nodes and pastes them into another canvas with remapped ids and edges', () => {
    ...
    state = canvasSlice.reducer(
      state,
      pasteNodes({ targetCanvasId: 'canvas-2', offset: { x: 50, y: 50 } }),
    );
    ...
    expect(targetCanvas?.nodes.every((node) => !['text-1', 'image-1'].includes(node.id))).toBe(
      true,
    );
