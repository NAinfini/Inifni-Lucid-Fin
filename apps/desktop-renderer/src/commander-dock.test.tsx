...
import type { PublicRunEvent } from '@lucid-fin/contracts';
import { App } from './App.js';
import { createDesktopApiFixture, resultFixture, runEventsFixture } from './test-fixture.js';
...
describe('Commander Dock', () => {
  it('owns the single inline Run surface and queues a follow-up without a second activity view', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <App api={fixture.api} createRequestId={() => 'request.ui.commander'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('enters Focus with the same conversation and restores the workspace on exit', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/canvas']}>
        <App api={fixture.api} createRequestId={() => 'request.ui.focus'} locale="en-US" />
      </MemoryRouter>,
    );
    ...
    try {
      ...
      render(
        <MemoryRouter initialEntries={['/projects/project.blue-hour/delivery']}>
          <App api={fixture.api} createRequestId={() => 'request.ui.narrow'} locale="en-US" />
        </MemoryRouter>,
      );
  ...
  it('resolves a published result to exact authority refs and undoes its current decision', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <App api={fixture.api} createRequestId={() => 'request.ui.quick'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('refreshes authoritative Result projections when a persisted result is published', async () => {
    ...
    fixture.calls.resultQuery.mockImplementation(async (request) => ({
      ...
      method: 'result.query',
      result: {
        items: request.input.query.resultIds.includes(resultFixture.resultRef.id)
          ? [resultFixture]
          : [],
        nextCursor: null,
      },
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <App api={fixture.api} createRequestId={() => 'request.ui.result-push'} locale="en-US" />
      </MemoryRouter>,
    );
    ...
    const event = runEventsFixture[1]!;
    for (const listener of fixture.listeners) {
      listener({
        wireVersion: 1,
        kind: 'push',
    ...
    await waitFor(() =>
      expect(
        fixture.calls.resultQuery.mock.calls.some(([request]) =>
          request.input.query.resultIds.includes(resultFixture.resultRef.id),
        ),
      ).toBe(true),
    );
  ...
  it('opens a Commander result in Media Candidates when Media is already showing Library', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/media']}>
        <App api={fixture.api} createRequestId={() => 'request.ui.open-result'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('shows the selected Result in the Focus inspector without duplicating Commander state', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/media']}>
        <App api={fixture.api} createRequestId={() => 'request.ui.inspector'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('protects the exact selected Result decision only after explicit confirmation', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/media']}>
        <App api={fixture.api} createRequestId={() => 'request.ui.protection'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('keeps authoritative History evidence inline in Overview and lets the user close it', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <App api={fixture.api} createRequestId={() => 'request.ui.history'} locale="en-US" />
      </MemoryRouter>,
    );
