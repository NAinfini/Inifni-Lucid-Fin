...
import { MediaPreview, useSynchronizedPlayback } from './MediaPreview.js';
import { createDesktopApiFixture, mediaFixture } from './test-fixture.js';
...
describe('Desktop media preview playback', () => {
  it('reuses a grant for semantically equivalent inline sources and refreshes changed sources', async () => {
    ...
    const preview = (refId: string) => (
      <DesktopEnvironmentProvider value={{ api: fixture.api, createRequestId, locale: 'en-US' }}>
        <MediaPreview
          projectId="project.blue-hour"
          source={{
            kind: 'project_media_ref',
            ref: { ...mediaFixture, id: refId },
          }}
          label="Reference preview"
        />
      </DesktopEnvironmentProvider>
    );
    const view = render(preview(mediaFixture.id));

    await waitFor(() => expect(fixture.calls.mediaPreviewIssue).toHaveBeenCalledTimes(1));
    view.rerender(preview(mediaFixture.id));
