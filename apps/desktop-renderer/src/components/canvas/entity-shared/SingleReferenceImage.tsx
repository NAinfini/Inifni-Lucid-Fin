...
import { VariantThumb } from './EntityThumbs.js';
...
interface SingleReferenceImageProps {
  ...
  entityId?: string;
  slot?: string;
  /** Tailwind aspect-ratio class for the image area, e.g. "aspect-[3/2]" or "aspect-[2/3]" */
  aspectRatio?: string;
  /** Label shown before the variant thumbnails row */
...
export function SingleReferenceImage({
  ...
  const resolvedVariantsLabel = variantsLabel ?? t('entity.variants');
  ...
  return (
    <div className="space-y-1.5">
      ...
      >
        {url ? (
          <div className={cn('relative w-full bg-muted rounded overflow-hidden', aspectRatio)}>
            <img
              src={url}
              alt="Reference"
          ...
          <div className={cn('flex flex-col items-center justify-center gap-2', aspectRatio)}>
            {isDragOver ? (
