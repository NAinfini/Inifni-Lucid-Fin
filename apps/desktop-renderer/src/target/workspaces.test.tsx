...
import {
  ...
  targetChatFixture,
  targetExportGrantFixture,
  targetHistoryFixture,
  targetMediaFixture,
...
function projectDelivery(fixture: ReturnType<typeof createTargetApiFixture>) {
...
function productionView(
  id: string,
  title: string,
): TargetResult<'production.query'>['items'][number] {
  return {
    object: {
...
function historyEntry(
  resultId: string,
  summary: string,
): TargetResult<'history.query'>['items'][number] {
  return { ...targetHistoryFixture[0], resultId, summary };
}
...
describe('target Project workspaces', () => {
  it('queries each authoritative projection and shares one selected object with Commander', async () => {
    ...
    expect((await screen.findByRole('img', { name: 'Harbor reference' })).getAttribute('src')).toBe(
      'lucid-target-media://preview/cap_fixture_project_media_ref',
    );
  ...
  it('surfaces authoritative generated-result decisions directly on Overview', async () => {
    ...
    expect(within(section!).getByText(targetResultFixture.resultRef.id)).toBeTruthy();
    expect(
      (
        await within(section!).findByLabelText(
          `Shot 04 · Harbor arrival · ${targetResultFixture.resultRef.id}`,
        )
      ).getAttribute('src'),
    ).toBe('lucid-target-media://preview/cap_fixture_generated_result');
  ...
  it('reviews generated candidates and undoes a decision from Compare with exact Choice and Shot refs', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/media']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.result'} locale="en-US" />
      </MemoryRouter>,
    );

    fireEvent.click(await screen.findByRole('tab', { name: 'Candidates' }));
    const panel = screen.getByRole('tabpanel');
    expect(within(panel).getByText(targetResultFixture.resultRef.id)).toBeTruthy();
    expect(
      (
  ...
  it('shows media preview failure and retries only after an explicit action', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/media']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.preview'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('fetches the latest historical Result when it falls beyond the first Result page', async () => {
    ...
    const firstPage = [1, 100].map((index) => {
      ...
      return {
        ...targetResultFixture,
        resultRef: { ...targetResultFixture.resultRef, id: `result.page.${ordinal}` },
        requestId: `request.page.${ordinal}`,
      };
    });
    const latestResult = {
      ...targetResultFixture,
      resultRef: { ...targetResultFixture.resultRef, id: 'result.page.101' },
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp
          api={fixture.api}
          createRequestId={() => 'request.ui.latest-result'}
          locale="en-US"
        />
      </MemoryRouter>,
    );
  ...
  it('retains the Result cursor and loads candidates beyond the first 100', async () => {
    ...
    fixture.calls.resultQuery.mockImplementation(async (request) => ({
      ...
      result:
        request.input.query.resultIds.length > 0
          ? { items: [targetResultFixture], nextCursor: null }
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/media']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.pages'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('loads more Project media with the original cursor, keeps selection, and retries a failed page', async () => {
    ...
    fixture.calls.mediaProjectList.mockImplementation(async (request) => {
      if (request.input.page.cursor === null) {
        return {
          ...
          result: { items: [targetMediaFixture], nextCursor: 'cursor.media.page.2' },
        };
      }
      pageAttempts += 1;
      ...
      return {
        ...
        requestId: request.requestId,
        method: 'media.project.list',
        result: { items: [targetMediaFixture, nextMedia], nextCursor: null },
      };
    });
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/media']}>
        <TargetApp
          api={fixture.api}
          createRequestId={() => 'request.ui.media-pages'}
          locale="en-US"
        />
      </MemoryRouter>,
    );
  ...
  it('reveals every loaded Overview history item and pages History without replacing existing evidence', async () => {
        ...
        items:
          request.input.query.resultIds.length === 0
            ...
            : request.input.query.resultIds.map((resultId) => ({
                ...targetResultFixture,
                resultRef: { ...targetResultFixture.resultRef, id: resultId },
              })),
        nextCursor: null,
      },
    }));
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp
          api={fixture.api}
          createRequestId={() => 'request.ui.history-pages'}
          locale="en-US"
        />
      </MemoryRouter>,
    );
  ...
  it('loads more Delivery plans with the original cursor, keeps selection, and retries a failed page', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/delivery']}>
        <TargetApp
          api={fixture.api}
          createRequestId={() => 'request.ui.delivery-pages'}
          locale="en-US"
        />
      </MemoryRouter>,
    );
  ...
  it('moves a Canvas placement through revision-checked canvas.apply', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/canvas']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.canvas'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('routes Review Cut preparation through the existing Commander Run', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/delivery']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.delivery'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('binds a selected export destination to the active Run follow-up and exact Delivery ref', async () => {
    ...
    expect(fixture.calls.exportPick.mock.calls[0]?.[0].input).toEqual({
      chatId: targetRunFixture.chatId,
    ...
    expect(request?.exportDestinationGrant).toEqual(targetExportGrantFixture);
    expect(request?.text).not.toContain(targetExportGrantFixture.destination.grantId);
  ...
  it('binds a selected export destination to a new root Run when the prior Run is terminal', async () => {
    ...
    fixture.calls.messageSend.mockImplementation(async (request) => ({
      ...
      result: {
        message: {
          ...(targetMessagesFixture[0] as Extract<
            (typeof targetMessagesFixture)[number],
            { role: 'user' }
          >),
    ...
    expect(request?.chatId).toBe(targetRunFixture.chatId);
    expect(request?.selectedContext).toEqual([
      {
        ref: {
          authority: 'delivery',
    ...
    expect(request?.exportDestinationGrant).toEqual(targetExportGrantFixture);
    expect(JSON.stringify(request?.blocks)).not.toContain(
      targetExportGrantFixture.destination.grantId,
    );
    expect(fixture.calls.runSendFollowup).not.toHaveBeenCalled();
  });
});
