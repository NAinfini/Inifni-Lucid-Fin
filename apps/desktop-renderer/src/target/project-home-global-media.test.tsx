...
describe('Target Project Home and Global Media', () => {
  it('offers Global Media as an available app-level destination', async () => {
    const fixture = createTargetApiFixture();
    render(
      <MemoryRouter initialEntries={['/projects']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.global-media'} />
      </MemoryRouter>,
    );
  ...
  it('imports and removes Global Media through the existing Target authorities', async () => {
    const fixture = createTargetApiFixture();
    render(
      <MemoryRouter initialEntries={['/media']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.global-media-commands'} />
      </MemoryRouter>,
    );
    ...
    fixture.calls.projectList.mockImplementation(async (request) => ({
      ...
      result:
        request.input.cursor === null
          ? { items: [targetProjectFixture], nextCursor: 'cursor.projects.page.2' }
          : { items: [secondProject], nextCursor: null },
    }));
    render(
      <MemoryRouter initialEntries={['/projects']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.projects-page'} />
      </MemoryRouter>,
    );
  ...
  it('loads additional Global Media pages and blocks mutation while the first page loads', async () => {
    ...
    let resolveFirstPage!: (value: Awaited<ReturnType<typeof fixture.calls.mediaGlobalList>>) => void;
    const firstPage = new Promise<Awaited<ReturnType<typeof fixture.calls.mediaGlobalList>>>(
      (resolve) => {
        resolveFirstPage = resolve;
      },
    );
    let resolveMorePage!: (value: Awaited<ReturnType<typeof fixture.calls.mediaGlobalList>>) => void;
    ...
    render(
      <MemoryRouter initialEntries={['/media']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.media-page'} />
      </MemoryRouter>,
    );

    expect((screen.getByRole('button', { name: 'Import Media' }) as HTMLButtonElement).disabled).toBe(
      true,
    );
    await waitFor(() => expect(fixture.calls.mediaGlobalList).toHaveBeenCalledTimes(1));
    await act(async () => {
      resolveFirstPage({
        ...
        result: { items: [targetGlobalMediaFixture], nextCursor: 'cursor.media.page.2' },
      });
  ...
  it('offers an explicit recovery path when Project setup fails after creation', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.project-recovery'} />
      </MemoryRouter>,
    );
  ...
  it('keeps Global Settings and metadata export visibly unavailable at their real authority boundary', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.authority-boundary'} />
      </MemoryRouter>,
    );
    ...
    expect(
      screen.getByText('Metadata export is not connected to a Target authority.'),
    ).toBeTruthy();
  });
  ...
  it('renames, archives, restores, and soft-deletes Projects only after a second confirmation', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.project-lifecycle'} />
      </MemoryRouter>,
    );
  ...
  it('presents Global Media authority failures as alerts', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/media']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.global-media-failure'} />
      </MemoryRouter>,
    );
