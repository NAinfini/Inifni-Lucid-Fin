...
import { TargetApiError, targetResult, type TargetResult } from './api.js';
...
function mediaProjectListInput(
  projectId: string,
  cursor: string | null,
): TargetDesktopCallV1<'media.project.list'>['input'] {
...
function productionQueryInput(
  projectId: string,
  cursor: string | null,
): TargetDesktopCallV1<'production.query'>['input'] {
  return {
    projectId,
    ids: [],
    types: [],
...
function historyQueryInput(
  projectId: string,
  cursor: string | null,
): TargetDesktopCallV1<'history.query'>['input'] {
  return {
    projectId,
    query: {
...
function deliveryQueryInput(
  projectId: string,
  cursor: string | null,
): TargetDesktopCallV1<'delivery.query'>['input'] {
...
): TargetDesktopCallV1<'result.query'>['input'] {
  return {
    projectId,
    query: {
      resultIds: [...resultIds],
...
function mergeResults(
  ...groups: ReadonlyArray<TargetWorkspaceData['results']>
): TargetWorkspaceData['results'] {
  const ids: string[] = [];
  const byId = new Map<string, TargetWorkspaceData['results'][number]>();
...
function mergeDeliveryPages(
  current: TargetWorkspaceData['delivery'],
  page: TargetResult<'delivery.query'>,
): TargetResult<'delivery.query'> {
...
type TargetHistoryEntry = TargetWorkspaceData['history'][number];
type ProtectionCommand = TargetDesktopCallV1<'decision.protect'>['input'];

function historyEntryKey(entry: TargetHistoryEntry): string {
...
function recentHistoryResultIds(history: readonly TargetHistoryEntry[]): string[] {
  const ids: string[] = [];
...
function FocusInspector({
  ...
  readonly entry: TargetHistoryEntry | null;
  ...
  if (entry !== null) {
    ...
    return (
      <div className="target-focus-inspector-content">
        <span className="target-inspector-kicker">
...
type TargetViewportMode = 'wide' | 'medium' | 'narrow';

function readViewportMode(): TargetViewportMode {
...
function ProjectSettingsTrigger({ onOpen }: { readonly onOpen: () => void }) {
  ...
  return (
    <button className="target-project-settings-trigger" type="button" onClick={onOpen}>
...
export function ProjectShell({
  ...
  const [capabilities, setCapabilities] = useState<TargetResult<'project.capabilities.get'> | null>(
    null,
  );
  const [capabilitiesError, setCapabilitiesError] = useState<string | null>(null);
  const [pluginPackages, setPluginPackages] = useState<TargetResult<'plugin.query'> | null>(null);
  ...
  const [inspectedHistory, setInspectedHistory] = useState<TargetHistoryEntry | null>(null);
  ...
  const [viewportMode, setViewportMode] = useState<TargetViewportMode>(readViewportMode);
  ...
  const [dockCollapsed, setDockCollapsed] = useState(() => {
    ...
    return initialViewportMode === 'narrow'
      ? true
      : readStoredBoolean(
          `lucid-fin:target:dock-collapsed:${projectId}`,
  ...
  const loadPluginPackages = useCallback(async () => {
    ...
    try {
      const next = await targetResult(
        api.plugin.query({ requestId: createRequestId(), input: {} }),
      );
  ...
  const loadDeliveryOperationViews = useCallback(
    async (operations: TargetResult<'delivery.query'>['operations']) => {
  ...
  const loadTranscript = useCallback(
    async (
      ...
      const page: TargetResult<'message.list'> = await targetResult(
        api.message.list({
          requestId: createRequestId(),
      ...
      let publicEvents: TargetResult<'run.events.list'> = { items: [], nextCursor: null };
      let clearPendingRunId: string | null = null;
      let ownershipWarning: string | null = null;
      if (runId !== null) {
        try {
          const loadedRun = await targetResult(
          ...
          if (loadedRun.projectId !== projectId || loadedRun.chatId !== chatId) {
            ...
            publicEvents = await targetResult(
              api.run.eventsList({
                requestId: createRequestId(),
          ...
          if (request !== transcriptRequestRef.current) return false;
          if (cause instanceof TargetApiError && cause.code === 'not_found') {
  ...
  const hydrate = useCallback(async () => {
    ...
    try {
      ...
      const storedChatId = readStoredString(`lucid-fin:target:last-chat:${projectId}`);
      const nextChat =
  ...
  useEffect(() => {
    if (loading) return;
    const key = `lucid-fin:target:last-chat:${projectId}`;
    try {
  ...
  useEffect(() => {
    ...
    const dispose = api.run.onEventsAppended((push) => {
      ...
      const current = event.runId === currentRun?.id;
  ...
  const prepareProjectSearch = async () => {
    ...
    while (nextChatCursor !== null) {
      ...
      const page: TargetResult<'chat.list'> = await targetResult(
        api.chat.list({
          requestId: createRequestId(),
          input: {
            projectId,
  ...
  const cancelDeliveryOperation = async (
    ...
    const cancelled = await targetResult(
  ...
  const resultDecisionStateForId = (resultId: string): TargetResultDecisionState => {
  ...
  const recordResultDecision = async (
    ...
    let input: TargetDesktopCallV1<'decision.record'>['input'];
    if (action === 'undo') {
    ...
    const choice = await targetResult(api.decision.record({ requestId: createRequestId(), input }));
  ...
  const requestProtection = async (
    ...
    try {
      const choice = await targetResult(
      ...
      if (cause instanceof TargetApiError && cause.confirmation !== null) {
        return {
  ...
  const respondProtection = async (
    ...
    const response = await targetResult(
  ...
  const applyPlugin = async (input: TargetDesktopCallV1<'plugin.apply'>['input']) => {
    if (pluginPendingRef.current !== null) return;
    pluginPendingRef.current = input.packageId;
    setPluginPending(input.packageId);
    try {
      await targetResult(api.plugin.apply({ requestId: createRequestId(), input }));
  ...
  const refreshNotice =
    refreshWarning === null ? null : (
      <div className="target-refresh-warning" role="alert">
  ...
  if (focus) {
    ...
    return (
      <div className={`target-focus-shell${hasInspector ? ' has-inspector' : ''}`}>
        {refreshNotice}
        <aside
    ...
    >
          ...
          {TARGET_WORKSPACES.map((item) => {
            ...
            return (
              <button
                ...
                aria-label={`${targetCopy(locale, item)} ${counts[item]}`}
      ...
      <section className="target-workspace-column">
        <header className="target-workspace-header">
          ...
          <div>
            <h1>{project.name}</h1>
            <span className="target-project-status">
          ...
          <button
            className="target-settings-toggle"
            type="button"
            onClick={() => setSettingsOpen(true)}
