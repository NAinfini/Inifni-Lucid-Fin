...
function messageListResponse(
  ...
  result: TargetResult<'message.list'>,
) {
  return {
...
function runEventsResponse(
  request: { readonly requestId: string },
  result: TargetResult<'run.events.list'>,
) {
  return {
    wireVersion: 1 as const,
    kind: 'success' as const,
...
describe('target Project shell', () => {
  ...
  it('keeps Global Media reachable from an open Project', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.shell-global-media'} />
      </MemoryRouter>,
    );
  ...
  it('archives an open Project through project.update before returning Home', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.shell-archive'} />
      </MemoryRouter>,
    );
    ...
    expect(fixture.calls.projectUpdate.mock.calls[0]?.[0].input).toMatchObject({
      ...
      expectedRevision: targetProjectFixture.revision,
      name: null,
      lifecycle: 'archived',
    });
  ...
  it('keeps an open Project visible when archiving is rejected', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.shell-archive-failure'} />
      </MemoryRouter>,
    );
  ...
  it('manages ready Providers and eligible Skills through the Project settings authority', async () => {
    ...
    const { container } = render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.settings'} locale="en-US" />
      </MemoryRouter>,
    );

    expect(await screen.findByRole('heading', { level: 1, name: 'Blue Hour' })).toBeTruthy();
    const settingsTrigger = container.querySelector<HTMLButtonElement>('.target-settings-toggle');
  ...
  it('refreshes only the owning Project overview for aggregate events from other Runs', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp
          api={fixture.api}
          createRequestId={() => 'request.ui.aggregate-push'}
          locale="en-US"
        />
      </MemoryRouter>,
    );
    ...
    appendRunEvent(
      fixture,
      event('event.current.terminal', targetRunFixture.id, {
        state: 'available',
        payload: {
  ...
  it('ignores a stale Project response after navigating to another Project', async () => {
    ...
    fixture.calls.projectGet.mockImplementation(async (request) => {
      if (request.input.projectId === targetProjectFixture.id) return firstProject;
      return {
        wireVersion: 1,
        kind: 'success',
        requestId: request.requestId,
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <ProjectSwitch />
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.switch'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('restores the last successful Chat selection for the Project', async () => {
    ...
    fixture.calls.chatList.mockImplementationOnce(async (request) => ({
      ...
      requestId: request.requestId,
      method: 'chat.list',
      result: { items: [targetChatFixture, secondChat], nextCursor: null },
    }));
    localStorage.setItem(
      `lucid-fin:target:last-chat:${targetProjectFixture.id}`,
      secondChat.id,
    );

    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.last-chat'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('deletes a Chat while its TaskList is active', async () => {
    ...
    const deleteChat = vi.fn(async (request) => ({
      ...
      result: {
        ...
        revision: targetChatFixture.revision + 1,
        contentHash: 'd'.repeat(64),
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={api} createRequestId={() => 'request.ui.delete-chat'} locale="en-US" />
      </MemoryRouter>,
    );
    ...
    expect(deleteChat.mock.calls[0]?.[0].input).toEqual({
      ...
      expectedRevision: targetChatFixture.revision,
    });
    await waitFor(() =>
      expect(
  ...
  it('archives a Chat through the same lifecycle authority', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={api} createRequestId={() => 'request.ui.archive-chat'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('uses the authoritative Chat revision after accepting a new root Run', async () => {
    ...
    const authoritativeChat = {
      ...targetChatFixture,
      revision: targetChatFixture.revision + 1,
      messageCount: targetChatFixture.messageCount + 1,
      messageHeadSequence: (targetChatFixture.messageHeadSequence ?? 0) + 1,
      contentHash: 'd'.repeat(64),
    };
    const acceptedMessage = {
      ...targetMessagesFixture[0],
      id: 'message.new-root-run',
      sequence: targetMessagesFixture[0].sequence + 1,
      contentHash: 'd'.repeat(64),
    } as Extract<(typeof targetMessagesFixture)[number], { role: 'user' }>;
    fixture.calls.runGet.mockImplementation(async (request) =>
      runResponse(request, { ...targetRunFixture, status: 'completed' } as Run),
    );
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={api} createRequestId={() => 'request.ui.authoritative-chat'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('does not apply a delayed root Run acceptance after switching and creating a Chat', async () => {
    ...
    const completedRun = { ...targetRunFixture, status: 'completed' } as Run;
    ...
    fixture.calls.messageList.mockImplementation(async (request) =>
      messageListResponse(
        ...
        request.input.chatId === secondChat.id
          ...
          : { items: targetMessagesFixture, nextCursor: null },
      ),
    );
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={api} createRequestId={() => 'request.ui.delayed-root'} />
      </MemoryRouter>,
    );

    await screen.findByText('Explore opening direction options for Shot 04. Keep the tone moody and grounded.');
    fireEvent.change(screen.getByPlaceholderText(/describe the next change/i), {
    ...
    await act(async () => {
      delayedSend.resolve({
        ...
        result: {
          message: acceptedMessage,
          chat: { ...targetChatFixture, revision: 2, contentHash: 'e'.repeat(64) },
          acceptedRun: targetRunFixture,
        },
      });
      await Promise.resolve();
    ...
    fireEvent.change(chatSelector, { target: { value: targetChatFixture.id } });
    await waitFor(() => expect((chatSelector as HTMLSelectElement).value).toBe(targetChatFixture.id));
    fireEvent.click(screen.getByRole('button', { name: 'Chat actions' }));
    fireEvent.click(screen.getByRole('button', { name: 'Archive Chat' }));
    await waitFor(() => expect(archiveChat).toHaveBeenCalledTimes(1));
    expect(archiveChat.mock.calls[0]?.[0].input).toEqual({
  ...
  it('keeps a failed Chat lifecycle command visible without removing the Chat', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp
          api={api}
          createRequestId={() => 'request.ui.delete-chat-failure'}
          locale="en-US"
        />
      </MemoryRouter>,
    );
  ...
  it('loads additional Chats without replacing the active Chat', async () => {
    ...
    fixture.calls.chatList.mockImplementation(async (request) => ({
      ...
      result:
        request.input.page.cursor === null
          ? { items: [targetChatFixture], nextCursor: 'cursor.chat.page.2' }
          : { items: [secondChat], nextCursor: null },
    }));
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.chat-page'} locale="en-US" />
      </MemoryRouter>,
    );
    ...
    expect((selector as HTMLSelectElement).value).toBe(targetChatFixture.id);
  ...
  it('loads earlier Messages and remaining Run events on demand', async () => {
    ...
    fixture.calls.runEventsList.mockImplementation(async (request) => ({
      ...
      result:
        request.input.page.cursor === null
          ? { items: [targetRunEventsFixture[0]], nextCursor: 'cursor.events.page.2' }
          : { items: [targetRunEventsFixture[1]], nextCursor: null },
    }));
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.timeline-page'} locale="en-US" />
      </MemoryRouter>,
    );
  ...
  it('reloads the authoritative transcript and Run after a follow-up', async () => {
    ...
    fixture.calls.messageList.mockImplementation(async (request) =>
      messageListResponse(request, {
        items: followupAccepted ? [...targetMessagesFixture, followupMessage] : targetMessagesFixture,
        nextCursor: null,
      }),
    );
    fixture.calls.runGet.mockImplementation(async (request) =>
      runResponse(
        request,
        followupAccepted
          ? { ...targetRunFixture, revision: targetRunFixture.revision + 1, contentHash: 'd'.repeat(64) }
          : targetRunFixture,
      ),
    );
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.authoritative-followup'} />
      </MemoryRouter>,
    );
  ...
  it('blocks a stale follow-up revision when its authoritative reload fails', async () => {
    ...
    fixture.calls.runGet.mockImplementation(async (request) => {
      ...
      return runResponse(request, targetRunFixture);
    });
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.followup-refresh-failure'} />
      </MemoryRouter>,
    );
    ...
    expect(await screen.findByText(/follow-up was accepted, but the Chat could not refresh/i)).toBeTruthy();
    fireEvent.change(screen.getByPlaceholderText(/describe the next change/i), {
  ...
  it('ignores stale Chat switches and paginated Messages or Run events', async () => {
    ...
    fixture.calls.runEventsList.mockImplementation(async (request) => {
      ...
      return runEventsResponse(request, {
        items: [targetRunEventsFixture[0]],
        nextCursor: 'cursor.events.page.2',
      });
    });
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.chat-race'} />
      </MemoryRouter>,
    );
  ...
  it('reloads changed run queries and rejects a Run outside the selected Project Chat', async () => {
    ...
    render(
      ...
      >
        ...
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.run-query'} />
      </MemoryRouter>,
    );
  ...
  it('surfaces non-not-found transcript Run failures instead of clearing the Run silently', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.run-failure'} />
      </MemoryRouter>,
    );

    expect((await screen.findByRole('alert')).textContent).toContain('Run authority is unavailable.');
  });
  ...
  it('preserves the current transcript when another Chat fails to load its Run', async () => {
    ...
    const secondMessage = {
      ...
      blocks: [{ type: 'text' as const, text: 'Second Chat should not replace the visible transcript.' }],
      contentHash: 'e'.repeat(64),
    };
    fixture.calls.chatList.mockImplementation(async (request) => ({
      wireVersion: 1 as const,
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.transcript-atomic'} />
      </MemoryRouter>,
    );
    ...
    expect((chatSelector as HTMLSelectElement).value).toBe(targetChatFixture.id);
    expect(
      screen.getByText('Explore opening direction options for Shot 04. Keep the tone moody and grounded.'),
    ).toBeTruthy();
  ...
  it('canonicalizes invalid workspace routes while preserving the query', async () => {
    const fixture = createTargetApiFixture();
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/invalid?chat=chat.opening-direction']}>
        <LocationProbe />
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.workspace-canonical'} />
      </MemoryRouter>,
    );
  ...
  it('opens a Project at overview when its stored workspace is invalid', async () => {
    const fixture = createTargetApiFixture();
    localStorage.setItem(`lucid-fin:target:last-workspace:${targetProjectFixture.id}`, 'obsolete-workspace');
    render(
      <MemoryRouter initialEntries={['/projects']}>
        <LocationProbe />
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.stored-workspace'} />
      </MemoryRouter>,
    );
  ...
  it('searches every Chat and Message page instead of silently stopping at page one', async () => {
    ...
    fixture.calls.chatList.mockImplementation(async (request) => ({
      ...
      result:
        request.input.page.cursor === null
          ? { items: [targetChatFixture], nextCursor: 'cursor.chat.search.2' }
          : { items: [secondChat], nextCursor: null },
    }));
    fixture.calls.messageList.mockImplementation(async (request) => ({
      wireVersion: 1,
    ...
    render(
      <MemoryRouter initialEntries={['/projects/project.blue-hour/overview']}>
        <TargetApp api={fixture.api} createRequestId={() => 'request.ui.complete-search'} />
      </MemoryRouter>,
    );
    ...
    expect(await screen.findByRole('button', { name: /Remote needle on the final page/ })).toBeTruthy();
    expect(
