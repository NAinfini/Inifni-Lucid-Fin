...
export type ResultDecisionAction = 'select' | 'reject' | 'refine' | 'use_as_reference' | 'undo';

export type ResultDecisionState = 'selected' | 'rejected' | 'refine' | 'reference' | null;

interface ResultDecisionControlsProps {
  readonly resultId: string;
  readonly state: ResultDecisionState;
  readonly disabledReason: string | null;
  readonly onDecide: (action: ResultDecisionAction, detail: string) => Promise<void>;
}
...
export function ResultDecisionControls({
  ...
  const decide = async (action: ResultDecisionAction, value = '') => {
  ...
  return (
    <div className="lucid-result-actions">
      {state !== null && (
        <span className={`lucid-result-decision is-${state}`} role="status">
          {state === 'selected'
            ? locale === 'zh-CN'
              ? '已选择'
              : 'Selected'
      ...
      <div className="lucid-result-action-row">
        <button
          type="button"
          disabled={disabled}
          title={actionTitle}
      ...
      {pending !== null && (
        <form
          className="lucid-result-detail-form"
          aria-label={
            pending === 'reject'
              ? locale === 'zh-CN'
                ? `拒绝结果 ${resultId}`
