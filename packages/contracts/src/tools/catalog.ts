...
import {
  ...
  ToolVersionSchema,
  type ToolDefinition as ContractToolDefinition,
  controlMetadata,
  defineTool,
  uniqueArray,
  variantExecution,
...
if (
  ...
  throw new Error(
    `Canonical Tool Catalog does not match the exact frozen ${EXACT_TOOL_IDS.length}-tool inventory`,
  );
}

type CatalogToolDefinition = ContractToolDefinition<
  string,
...
export type CatalogDefinition = (typeof TOOL_DEFINITIONS)[number];
