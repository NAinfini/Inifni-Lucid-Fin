...
function delegationCounts(database: ReturnType<typeof getTargetStoreDatabase>) {
...
function serializedDatabaseRows(database: ReturnType<typeof getTargetStoreDatabase>): string {
...
describe('Runs authority', () => {
  ...
  it('terminalizes atomically, maps an optional TaskList, rejects deep live descendants, and reopens', async () => {
    ...
    try {
      ...
      present.store.close();
      const reopened = await openTargetStore(present.databasePath);
      try {
  ...
  it('materializes the encrypted objective with the matching key and fails closed for key or envelope corruption', async () => {
    ...
    try {
      ...
      const missingKeyData = createTargetDataAccess(fixture.store, {
        now: () => NOW,
      ...
      const wrongKeyData = createTargetDataAccess(fixture.store, {
        now: () => NOW,
        createId: deterministicIds(),
        privateRecoveryCodec: createAes256GcmPrivateRecoveryCodec({
          encryptionKeyId: 'key.runs.private-recovery',
  ...
  it('rolls back after the parent event and reopens with deterministic exact replay', async () => {
    ...
    const reopened = await openTargetStore(fixture.databasePath);
    try {
      const data = createTargetDataAccess(reopened, {
        now: () => {
      ...
      expect(delegationCounts(getTargetStoreDatabase(reopened))).toEqual(beforeReopen);
      expect(
        data.runs.get({
          wireVersion: 1,
          kind: 'request',
