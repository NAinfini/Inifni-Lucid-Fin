...
function choiceActor(context: TargetCommandContext): Exclude<UserChoice['actor'], 'import'> {
...
function buildMutation(
  ...
  const exactItem = (ref: { id: string; revision: number; contentHash: string }) => {
    ...
    if (
      ...
      throw new TargetStorageError('REVISION_CONFLICT', `Delivery item ${ref.id} changed`);
    }
    return item;
  };
...
export function planDeliveryMutationInTransaction(
  ...
  if (request.input.action === 'create') {
    ...
    if (
      ...
      throw new TargetStorageError('REVISION_CONFLICT', `Project ${project.id} changed`);
    }
...
export function commitPlannedDeliveryMutationInTransaction(
  ...
  if (planned.before !== null) {
    ...
    if (canonicalJson(current) !== canonicalJson(planned.before)) {
      throw new TargetStorageError('REVISION_CONFLICT', `Delivery ${planned.before.id} changed`);
    }
  }
  const authorization = authorizeChoiceMutation(database, {
...
function decodeDeliveryToolCursor(cursor: string | null) {
  ...
  try {
    ...
    throw new TargetStorageError('INVALID_REQUEST', 'Delivery tool query cursor is invalid', {
      cause,
    });
  }
}
...
export interface DeliveryAuthority {
  ...
  readonly freeze: (input: FreezeDeliveryInput, context: TargetCommandContext) => DeliveryManifest;
...
export function createDeliveryAuthority(
  ...
  return Object.freeze({
    apply(request: ApplyRequest, context: TargetCommandContext, host?: CommandDispatchHost) {
    ...
    freeze(input: FreezeDeliveryInput, context: TargetCommandContext) {
