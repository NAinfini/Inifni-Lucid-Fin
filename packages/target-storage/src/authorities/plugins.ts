...
function queryRequest(inputValue: PluginQueryRequest): PluginQueryRequest {
  ...
  if (request.method !== 'plugin.query') {
    throw new TargetStorageError('INVALID_REQUEST', 'Expected plugin.query Wire request');
  }
...
function applyRequest(inputValue: PluginApplyRequest): PluginApplyRequest {
  ...
  if (request.method !== 'plugin.apply') {
    throw new TargetStorageError('INVALID_REQUEST', 'Expected plugin.apply Wire request');
  }
...
function trustedCatalog(port: TrustedPluginCatalogPort): readonly PluginPackageManifestV1[] {
  ...
  try {
    ...
    throw new TargetStorageError('INVALID_REQUEST', 'Trusted Plugin catalog is unavailable', {
      cause,
    });
  }
  const manifests = supplied.map((value) => parseManifest(value, 'catalog entry'));
  if (manifests.length > 500) {
    throw new TargetStorageError('INVALID_REQUEST', 'Trusted Plugin catalog exceeds 500 packages');
  }
  ...
  for (const manifest of manifests) {
    ...
    if (identities.has(identity)) {
      throw new TargetStorageError(
        'INVALID_REQUEST',
        `Trusted Plugin catalog duplicates ${identity}`,
      );
    }
...
function packageManifest(
  ...
  if (row === undefined) {
    throw new TargetStorageError(
      'NOT_FOUND',
      `Plugin package ${packageId}@${version} was not found`,
    );
  }
...
function removeInTransaction(
  ...
  if (manifest.manifestHash !== request.input.manifestHash) {
    throw new TargetStorageError('INVALID_REQUEST', 'Plugin package manifest hash does not match');
  }
  const current = installationForPackage(database, manifest.packageId);
  if (current === undefined || !installedManifestMatches(current, manifest)) {
    throw new TargetStorageError(
      'NOT_FOUND',
      `Plugin package ${manifest.packageId} is not installed`,
    );
  }
  if (request.input.expectedInstallationRevision !== current.revision) {
    throw new TargetStorageError(
...
export interface PluginPackagesAuthority {
  ...
  apply(request: PluginApplyRequest, context: TargetCommandContext): PluginApplySuccess;
}
...
export function createPluginPackagesAuthority(
  ...
  return Object.freeze({
    ...
    apply(requestValue: PluginApplyRequest, contextValue: TargetCommandContext) {
      ...
      if (context.actor !== 'user') {
        throw new TargetStorageError('INVALID_REQUEST', 'Only a user may manage Plugin packages');
      }
