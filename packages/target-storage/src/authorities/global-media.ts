...
function parseImportDescriptor(value: unknown): MediaImportDescriptor {
  try {
    ...
    throw new TargetStorageError('INVALID_REQUEST', 'Resolved Media descriptor is invalid', {
      cause,
    });
  }
}
...
function decodeCursor(cursor: string): z.output<typeof GlobalMediaListCursorSchema> {
  try {
    ...
    throw new TargetStorageError('INVALID_REQUEST', 'Global Media cursor is invalid', { cause });
  }
}
...
export function createGlobalMediaAuthority(
  ...
  const authority: GlobalMediaAuthority = {
    async importGlobal(requestValue, context) {
      ...
      >(getTargetStoreDatabase(store), request, context);
      ...
      if (
        ...
        throw new TargetStorageError('INVALID_REQUEST', 'Resolved Media capability is invalid');
      }
      ...
      if (stored.hash !== expected.hash || stored.byteLength !== expected.byteLength) {
        throw new TargetStorageError('CORRUPT_DATA', 'Media CAS returned a different object');
      }
    ...
    listGlobal(requestValue) {
      ...
      if (cursor !== null && cursor.filterHash !== filterHash) {
        throw new TargetStorageError(
          'INVALID_REQUEST',
          'Global Media cursor belongs to another query',
        );
      }
      const kindClause =
    ...
    getAsset(globalAssetId) {
      return loadGlobalMediaAsset(getTargetStoreDatabase(store), globalAssetId);
    },

    getBlob(blobHash) {
      return loadMediaBlob(getTargetStoreDatabase(store), blobHash);
    },

    getFolder(folderId) {
      return loadGlobalMediaFolder(getTargetStoreDatabase(store), folderId);
    },

    listFolders() {
      return listGlobalMediaFolders(getTargetStoreDatabase(store));
    },
  };
