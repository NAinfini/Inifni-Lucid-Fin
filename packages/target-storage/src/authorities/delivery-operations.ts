...
export function prepareReviewCutInTransaction(
  ...
  if (currentKey.fingerprint !== key.fingerprint) {
    throw new TargetStorageError('REVISION_CONFLICT', 'Review Cut input changed');
  }
  ...
  if (canonicalJson(currentManifest) !== canonicalJson(manifest)) {
    throw new TargetStorageError('REVISION_CONFLICT', 'Review Cut Manifest changed');
  }
...
function prepareExport(
  ...
  return withImmediateTransaction(database, () => {
    ...
    if (currentKey.fingerprint !== key.fingerprint) {
      throw new TargetStorageError('REVISION_CONFLICT', 'Delivery Export input changed');
    }
    const replay = findOperationByFingerprint(database, currentKey, existingDispatchOperationId);
    if (replay !== undefined && replay.operationKind !== null) {
      return {
  ...
  return Object.freeze({
    async preview(
      ...
      if (planRow === undefined)
        throw new TargetStorageError('NOT_FOUND', `Delivery was not found: ${plan.id}`);
