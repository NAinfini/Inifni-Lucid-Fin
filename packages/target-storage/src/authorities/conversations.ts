...
function decodeCursor<Schema extends z.ZodType>(
  ...
  try {
    ...
    throw new TargetStorageError('INVALID_REQUEST', 'List cursor is invalid', { cause });
  }
}
...
function queryMessages(
  ...
  if (defaultChat.projectId !== projectId) {
    throw new TargetStorageError('INVALID_REQUEST', 'Default Chat belongs to another Project');
  }
  const chat = input.chatId === null ? defaultChat : loadChat(database, input.chatId);
  if (chat.projectId !== projectId) {
    throw new TargetStorageError('INVALID_REQUEST', `Chat ${chat.id} belongs to another Project`);
  }
  ...
  if (cursor !== null && cursor.filterHash !== filterHash) {
    throw new TargetStorageError('INVALID_REQUEST', 'Chat query cursor belongs to another query');
  }
  ...
  return executeWireMutation(database, request, context, committedAt, () => {
    ...
    if (before.revision !== request.input.expectedRevision) {
      throw new TargetStorageError(
        'REVISION_CONFLICT',
        `Chat ${before.id} revision does not match`,
      );
    }
    if (
    ...
    if (Number(update.changes) !== 1) {
      throw new TargetStorageError('REVISION_CONFLICT', `Chat ${before.id} changed concurrently`);
    }
...
export function createConversationsAuthority(
  ...
  const authority: ConversationAuthority = {
    createChat(request, context) {
      ...
      return createChat(getTargetStoreDatabase(store), environment, parsed, context);
    },
    ...
    getChat(chatId) {
      return loadChat(getTargetStoreDatabase(store), chatId);
    },
    listChats(request) {
      ...
      if (cursor !== null && cursor.filterHash !== filterHash) {
        throw new TargetStorageError('INVALID_REQUEST', 'Chat cursor belongs to another query');
      }
    ...
    getMessage(messageId) {
      return loadMessage(getTargetStoreDatabase(store), messageId);
    },
    listMessages(request) {
      ...
      if (cursor !== null && cursor.filterHash !== filterHash) {
        throw new TargetStorageError('INVALID_REQUEST', 'Message cursor belongs to another Chat');
      }
    ...
    queryMessages(projectId, defaultChatId, input) {
      return queryMessages(getTargetStoreDatabase(store), projectId, defaultChatId, input);
    },
    sendMessage(request, context, seed) {
      const parsed = exactRequest(request, 'message.send');
      return acceptRootRunForMessage(
        getTargetStoreDatabase(store),
        environment,
        parsed,
        context,
