...
function requireExactProductionOwner(
  ...
  if (object.revision !== ref.revision || object.contentHash !== ref.contentHash) {
    throw new TargetStorageError('REVISION_CONFLICT', `Production object ${ref.id} changed`);
  }
  return object;
}

function choiceActor(context: TargetCommandContext): UserChoice['actor'] {
...
function requireExactDeliveryOwner(
  ...
  if (plan.revision !== ref.revision || plan.contentHash !== ref.contentHash) {
    throw new TargetStorageError('REVISION_CONFLICT', `Delivery ${ref.id} changed`);
  }
  return plan;
}
...
function persistUndoEffect(
  ...
  if (effect.active) {
    ...
    if (Number(released.changes) !== 1) {
      throw new TargetStorageError('REVISION_CONFLICT', 'Production protection changed');
    }
  }
}
...
function restoreDeliveryEffect(
  ...
  if (effect.order !== null) {
    ...
    if (expected.size !== active.length || active.some((item) => !expected.has(item.id))) {
      throw new TargetStorageError('REVISION_CONFLICT', 'Delivery undo order is no longer exact');
    }
...
function currentDeliveryChoiceFields(
  ...
  if (rows.length === 0) {
    throw new TargetStorageError('REVISION_CONFLICT', 'Delivery undo Choice is not current');
  }
  return rows.map((row) => {
...
function persistDeliveryProtectionUndo(
  ...
  if (effect.active) {
    ...
    if (Number(released.changes) !== 1) {
      throw new TargetStorageError('REVISION_CONFLICT', 'Delivery protection changed');
    }
  }
}
...
function planDeliveryUndoDecisionMutation(
  ...
  if (target.afterEffect.kind === 'delivery' && target.beforeEffect.kind === 'delivery') {
    if (
      ...
      throw new TargetStorageError(
        'REVISION_CONFLICT',
        'Delivery undo target is no longer current',
      );
    }
    ...
    if (
      ...
      throw new TargetStorageError(
        'REVISION_CONFLICT',
        'Delivery protection undo is no longer current',
      );
    }
    fields = [field];
...
function planProductionUndoDecisionMutation(
  ...
  if (
    ...
    throw new TargetStorageError('REVISION_CONFLICT', 'Undo target is not the exact current owner');
  }
  ...
  if (canonicalJson(current) !== canonicalJson(target.afterEffect)) {
    throw new TargetStorageError('REVISION_CONFLICT', 'Undo target is no longer current');
  }
  if (target.afterEffect.kind === 'result_decisions') {
    ...
    for (const entry of target.afterEffect.entries) {
      if (entry.value !== null && decisions.get(entry.resultId)?.currentChoiceId !== target.id) {
        throw new TargetStorageError('REVISION_CONFLICT', 'Undo target Choice is not current');
      }
    }
    ...
    if (active?.choiceId !== target.id) {
      throw new TargetStorageError('REVISION_CONFLICT', 'Undo protection Choice is not current');
    }
  }
...
function assertPlannedDecisionSnapshot(
  ...
  if (planned.before.authority === 'production') {
    ...
    if (canonicalJson(current) !== canonicalJson(planned.before)) {
      throw new TargetStorageError('REVISION_CONFLICT', `Production object ${current.id} changed`);
    }
    return;
  }
  ...
  if (canonicalJson(current) !== canonicalJson(planned.before)) {
    throw new TargetStorageError('REVISION_CONFLICT', `Delivery ${current.id} changed`);
  }
}
...
export function createUserChoicesAuthority(
  ...
  const authority: UserChoicesAuthority = {
    recordResultDecision(request, context, host) {
      ...
      return recordDecision(getTargetStoreDatabase(store), environment, exact, context, host);
    },
    setProtection(request, context, host) {
      const exact = exactRequest(request, 'decision.protect');
      return setDecisionProtection(
