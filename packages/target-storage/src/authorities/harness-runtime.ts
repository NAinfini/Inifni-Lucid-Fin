...
function pendingProtectedMutationSuspension(
  ...
  try {
    ...
    if (cause instanceof TargetStorageError && cause.code === 'REVISION_CONFLICT') return false;
    throw cause;
  }
  if (
...
function assertReplayQuote(
  ...
  if (
    ...
    throw new TargetStorageError(
      ...
      `Model Attempt ${attempt.id} quote changed`,
    );
  }
}
...
function prepareModelBoundary(
  ...
  if (existingRow !== undefined) {
    ...
    if (attempt.requestHash !== requestHash) {
      throw new TargetStorageError(
        'IDEMPOTENCY_CONFLICT',
        `Model Attempt ${attempt.id} request changed`,
      );
    }
...
function settleAgentSpawnBoundary(
  ...
  return withImmediateTransaction(database, () => {
    ...
    if (before.requestHash !== input.requestHash) {
      throw new TargetStorageError(
        'IDEMPOTENCY_CONFLICT',
        `Model Attempt ${before.id} request changed`,
      );
    }
...
function advanceToolProgramChild(
  ...
  const commandContext = parseCanonical(TargetCommandContextSchema, contextValue);
  return withImmediateTransaction(database, () => {
...
function settleToolProgramChildCall(
  database: DatabaseSync,
  _environment: TargetStorageEnvironment,
  ...
  parseCanonical(TargetCommandContextSchema, contextValue);
  return withImmediateTransaction(database, () => {
...
function startToolProgramChildActivation(
  ...
  const parentContext = parseCanonical(TargetCommandContextSchema, contextValue);
  const boundary = loadToolProgramBoundary(database, privateRecoveryCodec, parentDispatchId);
  const child = boundary.child.child;
  const childContext = parseCanonical(TargetCommandContextSchema, {
    ...parentContext,
...
function assertRecoveryRunIdentity(run: Run, input: CloseInterruptedActivationInput): void {
  if (
    ...
    throw new TargetStorageError('REVISION_CONFLICT', `Run ${run.id} changed before recovery`);
  }
...
export function createHarnessPersistenceAuthority(
  ...
  const authority: HarnessPersistenceAuthority = {
    ...
    prepareModelBoundary(input, context) {
      return prepareModelBoundary(getTargetStoreDatabase(store), environment, input, context);
    },
    markModelAttemptRunning(input, _context) {
      return markModelAttemptRunning(getTargetStoreDatabase(store), input);
    },
    settleModelAttempt(input, context) {
      return settleModelAttempt(getTargetStoreDatabase(store), environment, input, context);
    },
    ...
    settleAgentWaitStartBoundary(input, context) {
      return settleAgentWaitStartBoundary(
        getTargetStoreDatabase(store),
        environment,
        input,
        context,
    ...
    loadAgentWaitBoundary(dispatchOperationId) {
      return loadAgentWaitBoundary(getTargetStoreDatabase(store), environment, dispatchOperationId);
    },
    settleAgentWaitBoundary(input, context) {
      return settleAgentWaitBoundary(getTargetStoreDatabase(store), environment, input, context);
    },
    settleAgentResultBoundary(input, context) {
      return settleAgentResultBoundary(getTargetStoreDatabase(store), environment, input, context);
    },
    settleAgentCancelBoundary(input, context) {
      return settleAgentCancelBoundary(getTargetStoreDatabase(store), environment, input, context);
    },
    settleInteractionAskBoundary(input, context) {
      return settleInteractionAskBoundary(
        getTargetStoreDatabase(store),
        environment,
        input,
        context,
    ...
    settleDeliveryFreezeBoundary(input, context) {
      return settleDeliveryFreezeBoundary(
        getTargetStoreDatabase(store),
        environment,
        input,
        context,
    ...
    loadDeliveryExportBoundary(dispatchOperationId) {
      return loadDeliveryExportBoundary(getTargetStoreDatabase(store), dispatchOperationId);
    },
    settleDeliveryExportBoundary(input, context) {
      return settleDeliveryExportBoundary(
        getTargetStoreDatabase(store),
        environment,
        input,
        context,
    ...
    loadDeliveryPreviewBoundary(dispatchOperationId) {
      return loadDeliveryPreviewBoundary(getTargetStoreDatabase(store), dispatchOperationId);
    },
    settleDeliveryPreviewBoundary(input, context) {
      return settleDeliveryPreviewBoundary(
        getTargetStoreDatabase(store),
        environment,
        input,
        context,
    ...
    settleEvaluationRunStartBoundary(input, context) {
      return settleEvaluationRunStartBoundary(
        getTargetStoreDatabase(store),
        environment,
        input,
        context,
    ...
    loadEvaluationRunBoundary(dispatchOperationId) {
      return loadEvaluationRunBoundary(getTargetStoreDatabase(store), dispatchOperationId);
    },
    settleEvaluationRunBoundary(input, context) {
      return settleEvaluationRunBoundary(
        getTargetStoreDatabase(store),
        environment,
        input,
        context,
    ...
    loadGenerationSubmitBoundary(dispatchOperationId) {
      return loadGenerationSubmitBoundary(getTargetStoreDatabase(store), dispatchOperationId);
    },
    settleGenerationSubmitBoundary(input, context) {
      return settleGenerationSubmitBoundary(
        getTargetStoreDatabase(store),
        environment,
        input,
        context,
    ...
    settleMediaDeriveStartBoundary(input, context) {
      return settleMediaDeriveStartBoundary(
        getTargetStoreDatabase(store),
        environment,
        input,
        context,
    ...
    loadMediaDeriveBoundary(dispatchOperationId) {
      return loadMediaDeriveBoundary(getTargetStoreDatabase(store), dispatchOperationId);
    },
    settleMediaDeriveBoundary(input, context) {
      return settleMediaDeriveBoundary(getTargetStoreDatabase(store), environment, input, context);
    },
    settleMediaAttachBoundary(input, context) {
      return settleMediaAttachBoundary(getTargetStoreDatabase(store), environment, input, context);
    },
    settleMediaLinkBoundary(input, context) {
      return settleMediaLinkBoundary(getTargetStoreDatabase(store), environment, input, context);
    },
    settleCanvasMutateBoundary(input, context) {
      return settleCanvasMutateBoundary(getTargetStoreDatabase(store), environment, input, context);
    },
    settleOperationCancelBoundary(input, context) {
      return settleOperationCancelBoundary(
        getTargetStoreDatabase(store),
        environment,
        input,
        context,
    ...
    settleTaskManageBoundary(input, context) {
      return settleTaskManageBoundary(getTargetStoreDatabase(store), environment, input, context);
    },
    ...
    isRunActivationActive(runId, activationNumber) {
      return isRunActivationActive(getTargetStoreDatabase(store), runId, activationNumber);
    },
    ...
    prepareDispatch(input, context) {
      return prepareDispatch(getTargetStoreDatabase(store), environment, input, context);
    },
    prepareSkillProposal(input, context) {
      return prepareSkillProposal(getTargetStoreDatabase(store), environment, input, context);
    },
    prepareProtectedMutationBoundary(input, context) {
      return prepareProtectedMutationBoundary(
        getTargetStoreDatabase(store),
        environment,
        input,
        context,
    ...
    settleDispatch(input, context) {
      return settleDispatch(getTargetStoreDatabase(store), input, context, environment);
    },
    closeInterruptedActivation(input, context) {
      return closeInterruptedActivation(getTargetStoreDatabase(store), environment, input, context);
    },
    acceptCrashRetryRun(inputValue, contextValue) {
      const input = parseCanonical(AcceptCrashRetryRunInputSchema, inputValue);
      parseCanonical(TargetCommandContextSchema, contextValue);
      return acceptCrashRetryRootRun(getTargetStoreDatabase(store), environment, input);
    },
  };
