...
import { createTargetStore, openOrCreateTargetStore, openTargetStore } from './index.js';

const disposablePaths: string[] = [];

async function disposableDatabase(name = 'project.sqlite'): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'lucid-fin-target-store-'));
...
describe('target store lifecycle', () => {
  it('opens or creates one canonical store without replacing an existing database', async () => {
    ...
    const fresh = await openOrCreateTargetStore(databasePath);
    expect(fresh.created).toBe(true);
    const fingerprint = fresh.store.schemaFingerprint;
    fresh.store.close();

    const existing = await openOrCreateTargetStore(databasePath);
  ...
  it('creates a fully validated database by same-directory atomic rename and reopens it', async () => {
    const databasePath = await disposableDatabase();
    const created = await createTargetStore(databasePath);
    ...
    const schemaBindings = JSON.parse(
      await readFile(
        new URL('../../../target-contracts/generated/schema-bindings.v1.json', import.meta.url),
        'utf8',
      ),
    ) as { bindings: unknown[] };
  ...
  it('rejects unknown sqlite_schema drift before returning a writable store', async () => {
    ...
    await expect(openTargetStore(databasePath)).rejects.toMatchObject({ code: 'SCHEMA_DRIFT' });
  });
  ...
  it('rejects persisted foreign-key violations', async () => {
    ...
    await expect(openTargetStore(databasePath)).rejects.toMatchObject({
