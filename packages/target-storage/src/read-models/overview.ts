...
function exactRequest(value: OverviewRequest): OverviewRequest {
  ...
  if (request.method !== 'overview.get') {
    throw new TargetStorageError('INVALID_REQUEST', 'Expected overview.get Wire request');
  }
...
export function createProjectOverviewReadModel(store: TargetStore): ProjectOverviewReadModel {
  return Object.freeze({
    get(requestValue: OverviewRequest): OverviewSuccess {
      return overview(getTargetStoreDatabase(store), exactRequest(requestValue));
    },
  });
}
