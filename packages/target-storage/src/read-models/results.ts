...
function decodeResultsCursor(cursor: string | null) {
  ...
  try {
    ...
    throw new TargetStorageError('INVALID_REQUEST', 'Result query cursor is invalid', { cause });
  }
}
...
export function createProjectResultsReadModel(store: TargetStore): ProjectResultsReadModel {
    ...
    query(projectIdValue: string, inputValue: ResultQueryInput) {
      ...
      if (cursor !== null && cursor.filterHash !== filterHash) {
        throw new TargetStorageError(
          'INVALID_REQUEST',
          'Result query cursor belongs to another query',
        );
      }
