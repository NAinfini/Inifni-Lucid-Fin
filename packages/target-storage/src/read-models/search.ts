...
function decodeSearchCursor(cursor: string | null) {
  ...
  try {
    ...
    throw new TargetStorageError('INVALID_REQUEST', 'Project search cursor is invalid', { cause });
  }
}
...
export function createProjectSearchReadModel(store: TargetStore): ProjectSearchReadModel {
  return Object.freeze({
    query(projectIdInput: string, inputValue: ProjectSearchQueryInput) {
      ...
      if (cursor !== null && cursor.filterHash !== filterHash) {
        throw new TargetStorageError(
          'INVALID_REQUEST',
          'Project search cursor belongs to another query',
        );
      }
