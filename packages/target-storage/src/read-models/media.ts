...
function decodeMediaQueryCursor(cursor: string | null) {
  ...
  try {
    ...
    throw new TargetStorageError('INVALID_REQUEST', 'Media query cursor is invalid', { cause });
  }
}
...
export function createMediaQueryReadModel(store: TargetStore): MediaQueryReadModel {
    ...
    query(projectIdValue: string, inputValue: MediaQueryInput) {
      ...
      if (cursor !== null && cursor.filterHash !== filterHash) {
        throw new TargetStorageError(
          'INVALID_REQUEST',
          'Media query cursor belongs to another query',
        );
      }
      if (
        (input.integrity.length > 0 && !input.integrity.includes('unknown')) ||
