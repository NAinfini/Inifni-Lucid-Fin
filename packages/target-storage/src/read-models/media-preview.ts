...
export interface TargetMediaPreviewSource {
...
export interface TargetMediaPreviewSourceResolver {
  resolve(input: MediaPreviewIssueInputV1): TargetMediaPreviewSource;
}

function unavailableSource(): never {
  throw new TargetStorageError('NOT_FOUND', 'Media preview source was not found');
}

function corrupt(message: string): never {
  throw new TargetStorageError('CORRUPT_DATA', message);
}
...
export function createTargetMediaPreviewSourceResolver(
  store: TargetStore,
  mediaCas: MediaCas,
): TargetMediaPreviewSourceResolver {
  return Object.freeze({
