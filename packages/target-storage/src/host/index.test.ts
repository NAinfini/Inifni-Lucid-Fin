...
import type { TargetDataAccess } from '../kernel/data-access.js';
import { openTargetStore, type TargetStore } from '../kernel/store.js';
...
function run(data: TargetDataAccess, runId: string, suffix: string) {
...
function tableCounts(store: TargetStore) {
...
describe('I2-H0 host-only Provider and Skill provisioning', () => {
  ...
  it('freezes host-provisioned Project Skills without exposing the builder on TargetDataAccess', () => {
    const { store, database } = memoryStore();
    try {
      const host = createHostCatalogProvisioning(store, { now: () => NOW });
...
describe('K7B host Skill confirmation response', () => {
  ...
  it('keeps an approved Project Skill available to the next root catalog after a cold reopen', async () => {
    ...
    try {
      ...
      reopened = await openTargetStore(databasePath);
