...
export function canonicalTaskItems(items: readonly TaskItem[]): TaskItem[] {
  ...
  if (ordered.length !== normalized.length) {
    throw new TargetStorageError('INVALID_REQUEST', 'Task item tree is disconnected or cyclic');
  }
  return ordered;
}
...
export function insertTaskList(database: DatabaseSync, taskList: TaskList): void {
  if (!database.isTransaction) {
    throw new TargetStorageError('INVALID_REQUEST', 'TaskList insert requires a transaction');
  }
  const value = parseCanonical(TaskListSchema, taskList);
  if (hashContentObject(value) !== value.contentHash) {
    throw new TargetStorageError('INVALID_REQUEST', `TaskList ${value.id} hash is invalid`);
  }
  database
    .prepare(
...
export function replaceTaskList(database: DatabaseSync, before: TaskList, after: TaskList): void {
  if (!database.isTransaction) {
    throw new TargetStorageError('INVALID_REQUEST', 'TaskList update requires a transaction');
  }
  ...
  if (
    ...
    throw new TargetStorageError('INVALID_REQUEST', 'TaskList replacement identity is invalid');
  }
  ...
  if (Number(update.changes) !== 1) {
    throw new TargetStorageError('REVISION_CONFLICT', `TaskList ${before.id} changed concurrently`);
  }
