...
function notFound(label: string, id: string): TargetStorageError {
  return new TargetStorageError('NOT_FOUND', `${label} was not found: ${id}`);
}
...
export function appendMessageInTransaction(
  ...
  if (chat.lifecycle !== 'active') {
    throw new TargetStorageError('INVALID_REQUEST', `Chat ${chat.id} is not active`);
  }
