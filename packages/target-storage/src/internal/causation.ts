import { CausationRefSchema, parseCanonical, type CausationRef } from '@lucid-fin/target-contracts';
          ...
          : kind === 'import'
            ...
            : kind === 'direct_ui'
              ...
              : kind === 'run_inbox'
                ...
                : null;
  if (value === null) {
    throw new TargetStorageError('CORRUPT_DATA', `Unknown persisted causation kind: ${kind}`);
  }
  try {
    return parseCanonical(CausationRefSchema, value);
  } catch (cause) {
    throw new TargetStorageError('CORRUPT_DATA', 'Persisted causation is invalid', { cause });
  }
}
