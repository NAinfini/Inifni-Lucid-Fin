...
export function upsertProjectSearchDocument(
  ...
  if (searchText.length > 200_000) {
    throw new TargetStorageError('INVALID_REQUEST', 'Project search text exceeds its limit');
  }
  ...
  if (existing === undefined) {
    ...
    if (inserted === undefined) {
      throw new TargetStorageError('CORRUPT_DATA', 'Project search insert was not visible');
    }
    database
