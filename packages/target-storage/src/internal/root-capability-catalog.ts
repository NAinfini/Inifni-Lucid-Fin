...
function parseBaseCatalog(input: CapabilityCatalogSnapshotV1): CapabilityCatalogSnapshotV1 {
  ...
  try {
    ...
    throw new TargetStorageError('INVALID_REQUEST', 'Root capability base is invalid', { cause });
  }
...
export function buildRootCapabilityCatalog(
  ...
  if (project === undefined) {
    throw new TargetStorageError('NOT_FOUND', `Project ${input.projectId} was not found`);
  }
  if (project.lifecycle !== 'active') {
    throw new TargetStorageError('INVALID_REQUEST', `Project ${input.projectId} is not active`);
  }
