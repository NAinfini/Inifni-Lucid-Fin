...
export function listRunControlSubtree(
  ...
  if (rows.length === 0 || rows[0]?.id !== root.id) {
    throw new TargetStorageError('CORRUPT_DATA', `Run ${root.id} disappeared from its subtree`);
  }
  if (rows.length > maximumRuns) {
    throw new TargetStorageError(
      'INVALID_REQUEST',
      `Run ${root.id} subtree exceeds ${maximumRuns} Runs`,
    );
  }
  return rows.map(({ id, depth }) => {
    ...
    if (run.projectId !== root.projectId) {
      throw new TargetStorageError(
        'CORRUPT_DATA',
        `Run ${run.id} is outside Run ${root.id}'s Project`,
      );
    }
    return { run, depth };
  });
...
export function controlRunSubtreeInTransaction(
  ...
  const ordered =
    input.action === 'cancel'
      ? [...subtree].sort((left, right) => right.depth - left.depth || left.run.id.localeCompare(right.run.id))
      : [...subtree].sort((left, right) => left.depth - right.depth || left.run.id.localeCompare(right.run.id));
