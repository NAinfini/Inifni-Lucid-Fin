import { EntityIdSchema, RunSchema, parseCanonical, type Run } from '@lucid-fin/target-contracts';
...
export function advanceRunJournalHead(
  ...
  if (!database.isTransaction) {
    throw new TargetStorageError('INVALID_REQUEST', 'Run update requires an active transaction');
  }
  ...
  if (
    ...
    throw new TargetStorageError('CORRUPT_DATA', `Run ${before.id} Journal head is inconsistent`);
  }
  ...
  if (Number(update.changes) !== 1) {
    throw new TargetStorageError('REVISION_CONFLICT', `Run ${before.id} changed concurrently`);
  }
  return after;
}
...
export function advanceRunJournalAndPrivateRecoveryHead(
  ...
  if (
    ...
    throw new TargetStorageError('CORRUPT_DATA', `Run ${before.id} head is inconsistent`);
  }
  const nextWithoutHash = {
    ...before,
    revision: before.revision + 1,
