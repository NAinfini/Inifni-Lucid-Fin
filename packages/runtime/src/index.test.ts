...
} from '../../storage/test/i2h/fixture.js';
import {
  createStorageReadToolExecutor,
  coordinateRun,
  drainRequestedOperationCancellations,
  recoverActivation,
  runActivation,
  type RecoverActivationDependencies,
  type ModelAdapter,
  type ToolExecution,
  type ToolExecutor,
} from './index.js';
...
function toolExecution(projectId: string, toolId: ToolId, input: unknown): ToolExecution {
...
function recoveryDependencies(persistence: HarnessPersistenceAuthority) {
  ...
  } satisfies RecoverActivationDependencies;
...
describe('runtime', () => {
  ...
  it('atomically persists one interaction.ask and waits without using the generic executor', async () => {
    ...
    let executeCount = 0;
    try {
      const waiting = await runActivation(
        {
          persistence: fixture.data.harness,
          model,
          toolExecutor: fakeToolExecutor(STORAGE_READ_IDS, () => {
        ...
        runActivation(
          {
            persistence,
            model,
            toolExecutor: createStorageReadToolExecutor(fixture.data),
          },
          {
            runId,
            activationNumber: 1,
  ...
  it('rejects receipt-aware operation.get as a tool.program child before it can execute', async () => {
    ...
    const reads = createStorageReadToolExecutor(fixture.data);
    const executions: ToolExecution[] = [];
  ...
  it('settles a model agent.spawn and reloads parent-only private spawn objectives before the next model turn', async () => {
    ...
    try {
      const parentSnapshot = await runActivation(
        {
          persistence: fixture.data.harness,
          model: parentModel,
          toolExecutor: fakeToolExecutor(STORAGE_READ_IDS, () => {
  ...
  it('settles agent.send privately, resumes its parent after a pre-model crash, and delivers it only to the child', async () => {
    ...
    try {
      ...
      let quoteCount = 0;
      const crashAfterSendModel: ModelAdapter = {
        provider: { providerId: PROVIDER_ID, model: PROVIDER_MODEL, reasoningStrength: null },
        async quote(_request, privateContext) {
  ...
  it('yields a final stop to a follow-up queued while the model is streaming', async () => {
    ...
    try {
      const yielded = await runActivation(
        {
          persistence: fixture.data.harness,
          model,
          toolExecutor: fakeToolExecutor(STORAGE_READ_IDS, () => {
  ...
  it('runs a real history.query through two durable model attempts', async () => {
    ...
    try {
      ...
      const finalSnapshot = await runActivation(
        {
          persistence: fixture.data.harness,
          model,
          toolExecutor: createStorageReadToolExecutor(fixture.data),
  ...
  it('materializes generation.quote only after a successful tool.get dispatch', async () => {
    ...
    try {
      const completed = await runActivation(
        { persistence: fixture.data.harness, model, toolExecutor: executor },
        {
          runId,
          activationNumber: 1,
  ...
  it('leaves a missing frozen skill dispatch open for crash recovery', async () => {
    ...
    try {
      ...
      expect(
        recoverActivation(dependencies, recoveryInput(interrupted, context, 'skill')),
      ).toMatchObject({
        closed: {
  ...
  it('maps scoped storage-backed reads', async () => {
    ...
    try {
      const executor = createStorageReadToolExecutor({
        canvas: {
      ...
      ).toThrow('Storage read tool canvas.query@9.0.0 is unavailable');
      expect(() =>
        executor.execute(
          toolExecution(
            project.id,
      ...
      const missingExecutor = createStorageReadToolExecutor({
  ...
  it('rejects missing and ambiguous frozen skill identities', async () => {
    ...
    try {
      const missingExecutor = createStorageReadToolExecutor(missing.fixture.data);
      expect(() =>
        missingExecutor.execute({
          ...toolExecution(
            missing.project.id,
      ...
      const ambiguousExecutor = createStorageReadToolExecutor({
        ...
        runActivation(
          {
            persistence: fixture.data.harness,
            model,
            toolExecutor: createStorageReadToolExecutor({
              canvas: fixture.data.canvas,
              conversations: fixture.data.conversations,
              delivery: fixture.data.delivery,
              generation: fixture.data.generation,
  ...
  it('completes directly when the first Model Attempt stops without tool work', async () => {
    ...
    try {
      ...
      const after = await runActivation(
        {
          persistence: fixture.data.harness,
          model,
  ...
  it('closes a running Model Attempt and accepts exactly one queued retry without model seams', async () => {
    ...
    try {
      ...
      const first = recoverActivation(dependencies, input);
      ...
      const second = recoverActivation(dependencies, input);
  ...
  it('fails closed without replaying a committed undispatched receipt-aware operation.get', async () => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        expect(() => recoverActivation(dependencies, input)).toThrowError(
          expect.objectContaining({
