...
export interface ToolExecutor {
  ...
  readonly execute: (execution: ToolExecution) => RuntimeLoopOutcome | Promise<RuntimeLoopOutcome>;
}

export interface StorageReadDataAccess {
  readonly canvas: Pick<DataAccess['canvas'], 'queryTool'>;
  readonly conversations: Pick<DataAccess['conversations'], 'queryMessages'>;
  readonly delivery: Pick<DataAccess['delivery'], 'queryTool'>;
  readonly generation: Pick<DataAccess['generation'], 'quote'>;
  readonly history: Pick<DataAccess['history'], 'query'>;
  readonly mediaInspection: Pick<DataAccess['mediaInspection'], 'inspect'>;
  readonly media: Pick<DataAccess['media'], 'query'>;
  readonly memory: Pick<DataAccess['memory'], 'query'>;
  readonly operations: Pick<DataAccess['operations'], 'query'>;
  readonly production: Pick<DataAccess['production'], 'queryTool'>;
  readonly providerCapabilities: Pick<DataAccess['providerCapabilities'], 'query'>;
  readonly projects: Pick<DataAccess['projects'], 'getTool'>;
  readonly search: Pick<DataAccess['search'], 'query'>;
  readonly results: Pick<DataAccess['results'], 'query'>;
  readonly runReplay: Pick<DataAccess['runReplay'], 'get' | 'inspect'>;
}

const STORAGE_READ_TOOL_IDS = Object.freeze([
...
const STORAGE_INITIAL_TOOL_IDS = Object.freeze([
  HistoryQueryDefinition.id,
  MemoryQueryDefinition.id,
  ResultQueryDefinition.id,
  SkillLoadDefinition.id,
...
function frozenRunReplay(
  dataAccess: StorageReadDataAccess,
...
): ReturnType<StorageReadDataAccess['runReplay']['get']> {
...
export function createStorageReadToolExecutor(dataAccess: StorageReadDataAccess): ToolExecutor {
  return Object.freeze({
    toolIds: STORAGE_READ_TOOL_IDS,
    initialToolIds: STORAGE_INITIAL_TOOL_IDS,
    execute(execution: ToolExecution) {
      ...
      if (definition === undefined) {
        throw new Error(
          `Storage read tool ${execution.toolId}@${execution.toolVersion} is unavailable`,
        );
      }
      const parsedInput = definition.parseInput(execution.input);
      if (canonicalJson(parsedInput) !== canonicalJson(execution.input)) {
        throw new Error(
          `Storage read tool ${execution.toolId}@${execution.toolVersion} input is not canonical`,
        );
      }
...
function cancellationContext(runId: string): CommandContext {
  return {
    actor: 'commander',
...
function assertOperationCancellationDependencies(dependencies: RuntimeDependencies): void {
  const missing = [
...
async function collectOperationCancellationFailures(
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (cause instanceof StorageError && cause.code === 'REVISION_CONFLICT') continue;
      failures.push(cause);
    }
  }
  return failures;
...
export interface RunCoordinatorDependencies extends RuntimeDependencies {
  readonly runs: RunSchedulingAuthority;
}

export type CoordinateRunInput = Omit<RunActivationInput, 'activationNumber'>;
...
export function recoverActivation(
  { persistence }: RecoverActivationDependencies,
  input: RecoverActivationInput,
): RecoverActivationResult {
...
function assertInitialBoundary(
  ...
  if (
    ...
    throw new Error('Runtime requires the requested active running Activation');
  }
...
function continuesActivation(
  ...
  if (
    ...
    throw new Error('Runtime reloaded a different Run Activation');
  }
  ...
  if (snapshot.recoveryRequired) {
    throw new Error('Runtime cannot continue across an unresolved recovery frontier');
  }
  return true;
}
...
function materializedToolsFor(
  ...
  if (executor.toolIds.length === 0) {
    throw new Error('ToolExecutor must declare at least one toolId');
  }
  const executableToolIds = new Set<ToolId>();
  for (const toolId of executor.toolIds) {
    if (executableToolIds.has(toolId)) {
      throw new Error(`ToolExecutor declares duplicate toolId ${toolId}`);
    }
    ...
    if (
      ...
      throw new Error(`ToolExecutor toolId ${toolId} is not a live bounded R definition`);
    }
  ...
  if (executor.initialToolIds.length === 0) {
    throw new Error('ToolExecutor must initially materialize at least one toolId');
  }
  const toolIds = new Set<ToolId>();
  for (const toolId of executor.initialToolIds) {
    if (toolIds.has(toolId)) {
      throw new Error(`ToolExecutor initially materializes duplicate toolId ${toolId}`);
    }
    if (!executableToolIds.has(toolId)) {
      throw new Error(`ToolExecutor initially materializes unsupported toolId ${toolId}`);
    }
    toolIds.add(toolId);
  }
...
function childRunContext(context: CommandContext, childRunId: string): CommandContext {
  return {
    ...context,
...
async function runActivationWithPrivateContext(
  dependencies: RuntimeDependencies,
  input: RunActivationInput,
  privateContext: PrivateModelContext,
...
export async function runActivation(
  ...
  return runActivationWithPrivateContext(dependencies, input, privateContext);
}
...
export async function coordinateRun(
  ...
  if (active.length === 1) {
    ...
    if (
      ...
      let resumed = await runActivationWithPrivateContext(
        dependencies,
        {
          ...input,
          runId,
    ...
    const resumed = await runActivationWithPrivateContext(
      dependencies,
      {
        ...input,
        runId,
  ...
  for (;;) {
    ...
    let snapshot = await runActivationWithPrivateContext(
      dependencies,
      {
        ...input,
        runId,
