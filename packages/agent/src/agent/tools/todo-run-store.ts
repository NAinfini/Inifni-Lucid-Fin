/**
 * Run-scoped store backing the `todo.manage { action: 'set' }` / `todo.manage { action: 'update' }` tools.
 *
...
export class TodoRunStore {
  ...
  /** Current active snapshot, or null if no `todo.manage { action: 'set' }` has fired yet. */
  ...
  set(input: TodoSetInput): TodoSnapshot {
    ...
    if (trimmed.length < MIN_ITEMS || trimmed.length > MAX_ITEMS) {
      throw new TodoRunStoreError(
        `todo.manage { action: 'set' } requires between ${MIN_ITEMS} and ${MAX_ITEMS} non-empty items (got ${trimmed.length}).`,
        'items_range',
      );
    }
  ...
  update(input: TodoUpdateInput): {
    ...
    if (this.snapshot.todoId !== input.todoId) {
      throw new TodoRunStoreError(
        `todo.manage { action: 'update' } targeted todoId "${input.todoId}" but the active todo is "${this.snapshot.todoId}". Re-read the active id via a recent snapshot and retry.`,
        'mismatched_todo',
      );
    }
    ...
    for (const upd of input.updates) {
      ...
      if (!existing) {
        throw new TodoRunStoreError(
          `todo.manage { action: 'update' } referenced item id "${upd.id}" which is not in the active list.`,
          'unknown_id',
        );
      }
