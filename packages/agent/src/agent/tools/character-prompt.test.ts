...
describe('character prompt builders', () => {
  ...
  it('builds a full-sheet composite prompt for the default view', () => {
    ...
    expect(prompt).toContain('Professional character design reference sheet of Captain Mire');
    expect(prompt).toContain('six panels on one sheet arranged in two rows of three');
    expect(prompt).toContain('front view, left three-quarter profile, rear view');
    expect(prompt).toContain('neutral resting face, genuine warm smile, intense angry scowl');
    expect(prompt).toContain('~70% height');
    expect(prompt).toContain('~30% height');
    expect(prompt).toContain('Clean matte white cyclorama backdrop');
  ...
  it('prepends stylePlate as the first prompt segment for full-sheet', () => {
    ...
    const sheetIdx = prompt.indexOf('Professional character design reference sheet of');
