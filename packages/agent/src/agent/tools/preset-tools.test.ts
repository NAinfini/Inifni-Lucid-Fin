...
describe('createPresetTools', () => {
  it('defines the expected preset tool set', () => {
    ...
    expect(tools.map((tool) => tool.name)).toEqual(['preset.manage']);
  });
  ...
  it('lists, saves, resets, deletes, and gets presets', async () => {
    ...
    const customResult = await getTool('preset.manage', deps).execute({
      action: 'create',
      name: 'Dreamy Blur',
      category: 'look',
  ...
  describe('preset.manage list query and categories filter', () => {
    ...
    it('returns all presets when no filter is provided', async () => {
      ...
      const result = await getTool('preset.manage', deps).execute({ action: 'list' });
  ...
  describe('preset.manage get batch support', () => {
