...
describe('buildLocationRefImagePrompt', () => {
  it('builds a five-frame bible composite for the default view', () => {
    ...
    expect(prompt).toContain('Cinematic environment concept art bible of Old Arcade');
    ...
    expect(prompt).toContain('atmosphere and mood study');
    expect(prompt).toContain('primary key camera angle');
    expect(prompt).toContain('alternate camera angle');
  ...
  it('builds an 8-panel fake-360 pseudo-panorama', () => {
    ...
    expect(prompt).toContain('Cinematic environment pseudo-panorama of Old Arcade');
