    ...
    async execute(args) {
      try {
        if (!Array.isArray(args.nodes) || args.nodes.length === 0) {
          throw new TypedToolError(
            formatValidationError(
              ...
              args,
              'If you want to add a single node, pass a nodes array with one item. If you only want to connect existing nodes, call canvas.connectNodes.',
            ),
            'validation',
          );
        }
        ...
        if (args.nodes.length === 1 && edges.length === 0) {
          ...
          const nodeDesc = (args.nodes as Array<Record<string, unknown>>)[0];
          const singleArgs: Record<string, unknown> = {
            ...
            equipmentIds: nodeDesc.equipmentIds,
            position: nodeDesc.position,
          };
          const canvasId = requireString(singleArgs, 'canvasId');
          const type = requireCanvasNodeType(singleArgs);
          const title = requireString(singleArgs, 'title');
          const canvas = await requireCanvas(deps, canvasId);
          ...
          if (
            singleArgs.position &&
            typeof (singleArgs.position as Record<string, unknown>).x === 'number' &&
            typeof (singleArgs.position as Record<string, unknown>).y === 'number'
          ) {
            position = requirePosition(singleArgs);
          } else {
          ...
          if (type === 'text' && typeof singleArgs.content === 'string') {
            node.data = { content: singleArgs.content };
          }
          if (type !== 'text') {
            ...
            if (Array.isArray(singleArgs.characterIds)) {
              mediaData.characterRefs = (singleArgs.characterIds as string[]).map((id) => ({
                characterId: id,
              }));
            }
            if (Array.isArray(singleArgs.locationIds)) {
              mediaData.locationRefs = (singleArgs.locationIds as string[]).map((id) => ({
                locationId: id,
              }));
            }
            if (Array.isArray(singleArgs.equipmentIds)) {
              mediaData.equipmentRefs = (singleArgs.equipmentIds as string[]).map((id) => ({
                equipmentId: id,
              }));
            }
          }
          ...
          // Batch path: use batchCreate logic
          const canvasId = requireString(args, 'canvasId');
          const canvas = await requireCanvas(deps, canvasId);
  ...
  const getInfo: AgentTool = {
    ...
    parameters: {
      ...
      properties: {
        ...
        scope: {
          type: 'string',
          description: 'What to return. Default: "state".',
          enum: ['state', 'settings', 'all'],
        },
      },
      required: ['canvasId'],
    },
    async execute(args) {
      try {
        const canvasId = requireString(args, 'canvasId');
        const scope = typeof args.scope === 'string' ? args.scope : 'state';

        if (scope === 'settings') {
          await requireCanvas(deps, canvasId);
          if (!deps.getCanvasSettings) {
            return fail('canvas.getInfo { scope: "settings" } is not wired in this environment');
          }
          ...
          const settings = await deps.getCanvasSettings(canvasId);
          return ok({ ...stateResult, settings });
        } else {
          // scope === 'state' or undefined
          const canvas = await requireCanvas(deps, canvasId);
          return ok({
            id: canvas.id,
            name: canvas.name,
    ...
    parameters: {
      ...
      properties: {
        ...
        set: {
          type: 'object',
          description:
            'Fields to update (for updateBackdrop). ONLY include the fields you want to change — omitted fields are left untouched.',
          properties: {
