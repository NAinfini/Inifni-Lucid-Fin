...
describe('createRefImageTools', () => {
  ...
  describe('generateRefImage', () => {
    ...
    it('merges custom prompt with composite prompt (style plate + entity appearance + custom)', async () => {
      const config = createConfig({
        getCanvas: vi.fn(
          async () =>
            ({
      ...
      expect(config.buildPrompt).toHaveBeenCalled();
      expect(config.generateImage).toHaveBeenCalledWith(
        expect.stringContaining('Additional instructions: user custom prompt'),
        expect.anything(),
      );
    });
