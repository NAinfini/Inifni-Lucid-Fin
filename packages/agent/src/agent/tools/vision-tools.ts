...
export function createVisionTools(deps: VisionToolDeps): AgentTool[] {
  const describeImage: AgentTool = {
    ...
    description:
      "Analyze a node's current image asset using a vision AI model and return a detailed text prompt describing its style, content, and cinematic qualities. Optionally write the result to a node field. Use provider.manage { action: 'list', group: 'vision' } to see available providers.",
    tier: 2,
    parameters: {
      type: 'object',
      properties: {
        ...
        providerId: {
          type: 'string',
          description:
            "Optional: vision provider ID to use (e.g. \"openai-vision\", \"gemini-vision\"). Defaults to the first configured provider. Use provider.manage { action: 'list', group: 'vision' } to see available options.",
        },
        writeField: {
          type: 'string',
          enum: ['prompt'],
