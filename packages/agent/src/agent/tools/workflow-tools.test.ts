...
describe('createWorkflowTools', () => {
  it('defines workflow controls plus expandIdea', () => {
    ...
    expect(tools.map((tool) => tool.name)).toEqual(['workflow.manage']);
  ...
  it('delegates pause/resume/cancel/retry and validates ids', async () => {
    ...
    await expect(
      getTool(tools, 'workflow.manage').execute({ action: 'control', id: 'wf-1', controlAction: 'pause' }),
    ).resolves.toEqual({
      success: true,
      data: { id: 'wf-1', action: 'pause' },
    });
    await expect(
      getTool(tools, 'workflow.manage').execute({ action: 'control', id: 'wf-2', controlAction: 'resume' }),
    ).resolves.toEqual({
      success: true,
      data: { id: 'wf-2', action: 'resume' },
    });
    await expect(
      getTool(tools, 'workflow.manage').execute({ action: 'control', id: 'wf-3', controlAction: 'cancel' }),
    ).resolves.toEqual({
      success: true,
      data: { id: 'wf-3', action: 'cancel' },
    });
    await expect(
      getTool(tools, 'workflow.manage').execute({ action: 'control', id: 'wf-4', controlAction: 'retry' }),
    ).resolves.toEqual({
      success: true,
    ...
    await expect(
      getTool(tools, 'workflow.manage').execute({ action: 'control', id: ' ', controlAction: 'pause' }),
    ).resolves.toEqual({
      success: false,
  ...
  it('returns a teaching validation error for empty-arg expandIdea (04-19 fake-user-study fix)', async () => {
    const tools = createWorkflowTools(createDeps());
    const result = await getTool(tools, 'workflow.manage').execute({ action: 'expandIdea' });
    expect(result.success).toBe(false);
    if (result.success === false) {
      expect(result.error).toContain('workflow.manage');
      expect(result.error).toContain('expandIdea');
