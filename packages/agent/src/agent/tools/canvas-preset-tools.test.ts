...
describe('createCanvasPresetTools', () => {
  ...
  it('reads and writes full preset tracks for visual nodes', async () => {
    ...
    await expect(
      getTool('canvas.presetTracks', deps).execute({
        action: 'read',
        canvasId: 'canvas-1',
        nodeId: 'image-1',
      }),
  ...
  it('writes multiple categories in one call via tracks object', async () => {
    const canvas = createCanvas();
    const deps = createDeps(canvas);

    const result = await getTool('canvas.presetTracks', deps).execute({
      action: 'writeBatch',
      canvasId: 'canvas-1',
      nodeId: 'image-1',
      tracks: {
        camera: {
          intensity: 80,
          entries: [{ presetId: 'camera-push-in' }],
        },
        look: {
          intensity: 50,
        },
        ...
        tracks: [
          {
            ...
            track: expect.objectContaining({
              intensity: 80,
              entries: [expect.objectContaining({ presetId: 'camera-push-in' })],
            }),
          },
          {
            category: 'look',
  ...
  it('adds, updates, and removes single preset entries', async () => {
    ...
    const addResult = await getTool('canvas.presetTracks', deps).execute({
      action: 'addEntry',
      canvasId: 'canvas-1',
      nodeId: 'image-1',
      category: 'camera',
    ...
    await getTool('canvas.presetTracks', deps).execute({
      action: 'addEntry',
      canvasId: 'canvas-1',
      nodeId: 'image-1',
      category: 'camera',
...
describe('shotTemplate.manage', () => {
  it('lists all shot templates', async () => {
    const deps = createDeps();
    const result = await getTool('shotTemplate.manage', deps).execute({ action: 'list' });
...
describe('shotTemplate.manage (create)', () => {
