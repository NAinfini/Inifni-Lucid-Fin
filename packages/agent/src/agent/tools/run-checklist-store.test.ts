...
import { RunChecklistStore, RunChecklistStoreError } from './run-checklist-store.js';

function makeStore(
  overrides?: Parameters<typeof RunChecklistStore>[0] extends infer _ ? never : never,
) {
  void overrides;
  let seq = 0;
  const onSnapshot = vi.fn();
  const onUpdate = vi.fn();
  const store = new RunChecklistStore({
...
describe('RunChecklistStore.set', () => {
  it('creates a snapshot with the first item in_progress and fires onSnapshot', () => {
    ...
    expect(snapshot.checklistId).toBe('checklist-1');
  ...
  it('rejects below-minimum item counts', () => {
    ...
    expect(() => store.set({ items: [{ label: 'only one' }] })).toThrow(RunChecklistStoreError);
  });
...
describe('RunChecklistStore.update', () => {
  ...
  it('rejects updates before any runChecklist.manage set', () => {
    const { store } = makeStore();
    expect(() =>
      store.update({ checklistId: 'ghost', updates: [{ id: 'x', status: 'done' }] }),
    ).toThrow(/called before runChecklist\.manage/);
  });

  it('rejects stale checklistId (drift guard)', () => {
    const { store } = makeStore();
    const first = store.set({ items: [{ label: 'a' }, { label: 'b' }] });
    store.set({ items: [{ label: 'x' }, { label: 'y' }] });
    expect(() =>
      store.update({ checklistId: first.checklistId, updates: [{ id: 'any', status: 'done' }] }),
    ).toThrow(/mismatched_checklist|active checklist is/);
  });
