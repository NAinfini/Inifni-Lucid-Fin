...
describe('createCanvasTools', () => {
  it('addNode creates node with correct defaults', async () => {
    ...
    const result = await getTool('canvas.createNodes', deps).execute({
      canvasId: 'canvas-1',
      nodes: [{ type: 'image', title: 'Generated Image', position: { x: 100, y: 200 } }],
    });
  ...
  it('updates backdrop style fields through updateBackdrop', async () => {
    ...
    await expect(
      getTool('canvas.manage', deps).execute({
        action: 'updateBackdrop',
        canvasId: 'canvas-1',
        nodeId: 'backdrop-1',
        set: {
          opacity: 72,
  ...
  describe('canvas.createNodes empty-arg validation (04-19 fake-user-study fix)', () => {
    it('rejects empty args and teaches the model what to send next', async () => {
      ...
      if (result.success === false) {
        expect(result.error).toContain('canvas.createNodes');
