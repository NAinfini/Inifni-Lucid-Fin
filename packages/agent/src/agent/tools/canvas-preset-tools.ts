    ...
    parameters: {
      ...
      properties: {
        ...
        nodeIds: {
          ...
          description: 'Batch: array of node IDs. Used by addEntry and applyTemplate batch modes.',
        },
        category: {
          type: 'string',
        ...
        intensity: {
          type: 'number',
          description: 'Category-level intensity (0-100). Used by write and addEntry.',
        },
        entries: {
          type: 'array',
          description: 'Replacement entries for write action.',
          items: {
            type: 'object',
        ...
        templateName: {
          type: 'string',
          description:
            'The template name to search for (case-insensitive partial match). Used with nodeId/nodeIds in applyTemplate.',
        },
        nodes: {
          type: 'array',
          description:
            'Per-node template assignments with different templates. Each entry has nodeId + templateName. Used by applyTemplate.',
          items: {
            type: 'object',
  ...
  const shotTemplateManage: AgentTool = {
    ...
    parameters: {
      ...
      properties: {
        ...
        templateId: { type: 'string', description: 'The template ID. Used by update and delete.' },
      },
      required: ['action'],
    },
    async execute(args) {
      ...
      return fail(new Error(`Unknown action "${action}". Valid: list, create, update, delete`));
    },
  };

  return [presetTracks, shotTemplateManage];
}
