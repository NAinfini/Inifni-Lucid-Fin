...
export interface CommitRequirement {
   ...
   * tool name alone is not enough — e.g. `canvas.createNodes` counts
...
export interface CompletionContract {
  ...
  /**
   * Alternates that also satisfy the contract (e.g. `canvas.createNodes`
   * with a single node for a small scene). The engine accepts a substitute
   * for any required commit whose `toolName` matches the substitute's
   * `toolName`.
   */
