...
function describeEvidence(e: CompletionEvidence): string {
  switch (e.kind) {
    ...
    case 'generation_started':
      return `gen:${e.nodeId}`;
    case 'settings_write':
      return `settings:${e.canvasId}:${e.keys.join(',')}`;
    case 'user_refused':
      return `refused:${e.message}`;
    case 'budget_exhausted':
      return `budget:${e.metric}`;
    case 'progress_stall':
      return `stall:${e.stepsSinceLastMutation}`;
    case 'tool_retry_loop':
      return `retry:${e.toolName}:${e.attempts}`;
    default:
      return assertNeverEvidence(e);
  }
}
...
function describeBlocker(b: BlockerReason): string {
  switch (b.kind) {
    case 'missing_commit':
      return `miss:${b.expected.join('|')}:${b.lastTool ?? '-'}`;
    case 'ask_user_loop':
      return `askLoop:${b.askCount}/${b.limit}`;
    case 'empty_narration':
      return `narr:${b.lastAssistantText.slice(0, 20)}`;
    case 'progress_stall':
      return `stall:${b.stepsSinceLastMutation}`;
    default:
      return assertNeverBlocker(b);
  }
}
...
function describeDecision(d: ExitDecision): string {
  switch (d.outcome) {
    case 'satisfied':
      return `ok:${d.contractId}`;
    case 'informational_answered':
      return `info:${d.reason}`;
    case 'blocked_waiting_user':
      return `block:${d.question}`;
    case 'refused':
      return `refused:${d.reason}`;
    case 'budget_exhausted':
      return `budget:${d.metric}`;
    case 'unsatisfied':
      return `unsat:${d.contractId}:${describeBlocker(d.blocker)}`;
    case 'error':
      return `err:${d.message}`;
    default:
      return assertNeverDecision(d);
  }
}
...
describe('exit-contract/types — BlockerReason', () => {
  const samples: BlockerReason[] = [
    { kind: 'missing_commit', expected: ['canvas.createNodes'] },
    { kind: 'missing_commit', expected: ['canvas.setSettings'], lastTool: 'canvas.getInfo' },
    { kind: 'ask_user_loop', askCount: 6, limit: 3 },
    { kind: 'empty_narration', lastAssistantText: 'I will now proceed.' },
    { kind: 'progress_stall', stepsSinceLastMutation: 4 },
  ];
      ...
      successSignals: [
        {
          ...
          check: (ledger) =>
            ledger.some(
              (e) =>
                e.kind === 'mutation_commit' && e.toolName === 'canvas.createNodes' && e.resultOk,
            ),
...
describe('exit-contract/types — assertNever helpers', () => {
  it('throws when handed a concrete value (runtime safety net)', () => {
    ...
    expect(() =>
      assertNeverDecision({ outcome: 'not_a_real_outcome' } as unknown as never),
    ).toThrow(/Unhandled ExitDecision/);
  });
});
