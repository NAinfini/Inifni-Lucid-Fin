 ...
 * `canvas.generation` is pending. Reminds the LLM to verify and expand
...
export function promptQualityGatePredicate(ctx: ActivationContext): boolean {
  ...
  return ctx.pendingToolCalls.some((tc) => {
    if (tc.name !== 'canvas.generation') return false;
    // When entity refs are attached, a short prompt is intentional —
    // the refs carry the visual load.
    const charRefs = tc.arguments?.characterRefs;
    const locRefs = tc.arguments?.locationRefs;
    if (
      (Array.isArray(charRefs) && charRefs.length > 0) ||
      (Array.isArray(locRefs) && locRefs.length > 0)
    ) {
      return false;
    }
