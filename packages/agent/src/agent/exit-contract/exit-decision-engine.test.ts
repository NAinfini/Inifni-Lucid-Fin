...
describe('exit-contract/decide', () => {
  ...
  describe('satisfied', () => {
    it('returns satisfied when required commit matches', () => {
      ...
      if (d.outcome === 'satisfied') {
        ...
        expect(d.evidenceSummary).toContain('canvas.createNodes');
      }
    });

    it('returns satisfied via acceptable substitute', () => {
      const ledger: ReadonlyCompletionEvidenceList = [
        { kind: 'mutation_commit', toolName: 'canvas.manage', args: {}, resultOk: true, at: 1 },
      ];
      const d = decide({ contract: execContract(), intent: execIntent, ledger });
      expect(d.outcome).toBe('satisfied');
    });
  ...
  describe('unsatisfied blockers', () => {
    ...
    it('missing_commit blocker lists expected tools and last tool called', () => {
      const ledger: ReadonlyCompletionEvidenceList = [
        { kind: 'mutation_commit', toolName: 'canvas.getInfo', args: {}, resultOk: true, at: 1 },
      ];
      const d = decide({ contract: execContract(), intent: execIntent, ledger });
      expect(d.outcome).toBe('unsatisfied');
      if (d.outcome === 'unsatisfied') {
        expect(d.blocker).toEqual({
          kind: 'missing_commit',
          expected: ['canvas.createNodes'],
          lastTool: 'canvas.getInfo',
        });
      }
    });
  });
  ...
  describe('wildcard toolName matching', () => {
    it("contract with toolName '*' matches any successful commit", () => {
      const wildcardContract = execContract({
        requiredCommits: [{ toolName: '*', description: 'Any mutation' }],
        acceptableSubstitutes: [],
      });
      const ledger: ReadonlyCompletionEvidenceList = [
        { kind: 'mutation_commit', toolName: 'preset.manage', args: {}, resultOk: true, at: 1 },
      ];
      const d = decide({ contract: wildcardContract, intent: execIntent, ledger });
      expect(d.outcome).toBe('satisfied');
    });

    it("contract with toolName '*' does not match a failed commit", () => {
      const wildcardContract = execContract({
        requiredCommits: [{ toolName: '*', description: 'Any mutation' }],
        acceptableSubstitutes: [],
      });
      const ledger: ReadonlyCompletionEvidenceList = [
        { kind: 'mutation_commit', toolName: 'preset.manage', args: {}, resultOk: false, at: 1 },
      ];
      const d = decide({ contract: wildcardContract, intent: execIntent, ledger });
      expect(d.outcome).toBe('unsatisfied');
    });
