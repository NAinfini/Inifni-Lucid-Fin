...
describe('createUtilityWorkflowTools', () => {
  ...
  it('classifies uploaded file content', async () => {
    const tools = createUtilityWorkflowTools();
    const classify = getTool(tools, 'file.classify');

    await expect(classify.execute({
    ...
    await expect(classify.execute({
      content: 'SYSTEM: You are a director',
      filename: 'prompt.txt',
    })).resolves.toEqual({
      success: true,
      data: expect.objectContaining({
    ...
    await expect(classify.execute({
      content: 'workflow: storyboard-export',
