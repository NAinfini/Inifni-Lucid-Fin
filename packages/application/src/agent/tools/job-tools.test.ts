...
describe('createJobTools', () => {
  ...
  it('delegates cancel, pause, and resume operations via job.control', async () => {
    ...
    await expect(
      getTool('job.control', deps).execute({ jobId: 'job-1', action: 'cancel' }),
    ).resolves.toEqual({
      success: true,
      data: { jobId: 'job-1', action: 'cancel' },
    });
    await expect(
      getTool('job.control', deps).execute({ jobId: 'job-2', action: 'pause' }),
    ).resolves.toEqual({
      success: true,
      data: { jobId: 'job-2', action: 'pause' },
    });
    await expect(
      getTool('job.control', deps).execute({ jobId: 'job-3', action: 'resume' }),
    ).resolves.toEqual({
      success: true,
  ...
  it('validates job IDs and wraps dependency failures', async () => {
    ...
    await expect(
      getTool('job.control', deps).execute({ jobId: '', action: 'cancel' }),
    ).resolves.toEqual({
      success: false,
    ...
    await expect(
      getTool('job.control', deps).execute({ jobId: 'job-9', action: 'pause' }),
    ).resolves.toEqual({
      success: false,
      error: 'job missing',
    });
  ...
  it('rejects unknown actions', async () => {
    ...
    await expect(
      getTool('job.control', deps).execute({ jobId: 'job-1', action: 'explode' }),
    ).resolves.toEqual({
      success: false,
