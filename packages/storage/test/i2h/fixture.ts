...
import { getStoreDatabase } from '../../dist/internal/database-access.js';
...
export const userContext: CommandContext = {
  actor: 'user',
  causation: { kind: 'direct_ui', actionId: 'action.i2h.journey' },
  correlationId: 'correlation.i2h.journey',
};

export function commanderContext(runId: string): CommandContext {
  return {
    actor: 'commander',
...
export function createJourneyDataAccess(
  ...
  const options: DataAccessOptions = {
    now: () => NOW,
    createId,
    privateRecoveryCodec,
  ...
  return createDataAccess(store, options);
}

export function getJourneyTestDatabase(store: Store) {
  return getStoreDatabase(store);
}
