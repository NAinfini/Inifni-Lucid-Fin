...
function countExposure(
  ...
  if (value === null || value > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new StorageError('CORRUPT_DATA', `Run ${runId} ${kind} exposure is invalid`);
  }
...
function recoveryCandidates(database: DatabaseSync, projectIdValue: string): Run[] {
  ...
  if (ordered.length !== rows.length) {
    throw new StorageError('CORRUPT_DATA', `Project ${projectId} Run lineage is invalid`);
  }
  return ordered
...
export function createRunReplayReadModel(store: Store): RunReplayReadModel {
  const readModel: RunReplayReadModel = {
    get(runId) {
      return projection(getStoreDatabase(store), runId);
    },
    inspect(runId, input) {
      return inspectProjection(getStoreDatabase(store), runId, input);
    },
    listRecoveryCandidates(projectId) {
      return recoveryCandidates(getStoreDatabase(store), projectId);
    },
  };
