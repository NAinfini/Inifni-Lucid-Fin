      ...
      : ref.authority === 'project_media_ref'
        ...
        : ref.authority === 'production'
          ...
          : (database
              ...
              { project_id: string; revision: number; content_hash: string } | undefined);
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Memory source was not found: ${ref.authority}:${ref.id}`);
  }
  if (
...
function loadMemoryIndex(database: DatabaseSync, memoryVersionId: string): ProjectMemoryIndex {
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Project Memory was not found: ${memoryVersionId}`);
  }
...
export function createProjectMemoryReadModel(store: Store): ProjectMemoryReadModel {
  return Object.freeze({
    recordVersion(index: ProjectMemoryIndex) {
      return recordVersion(getStoreDatabase(store), index);
    },
    publishHead(input: PublishMemoryHeadInput) {
      return publishHead(getStoreDatabase(store), input);
    },
    ...
    query(projectId: string, input: MemoryQueryInput) {
      return queryMemory(getStoreDatabase(store), projectId, input);
    },
  });
}
