...
function decodeCursor(cursor: string): z.output<typeof HistoryCursorSchema> {
  try {
    ...
    throw new StorageError('INVALID_REQUEST', 'History cursor is invalid', { cause });
  }
}
...
export function createProjectHistoryReadModel(store: Store): ProjectHistoryReadModel {
  return Object.freeze({
    query(
      ...
      if (cursor !== null && (cursor.filterHash !== filterHash || cursor.order !== order)) {
        throw new StorageError('INVALID_REQUEST', 'History cursor belongs to another query');
      }
      const entries = [
