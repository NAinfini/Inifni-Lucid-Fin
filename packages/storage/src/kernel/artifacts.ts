...
const canonicalContractsRoot = new URL('../../../contracts/', import.meta.url);
...
interface BuiltInSkillPackDocumentV1 {
  readonly version: 1;
  readonly skills: readonly SkillDocument[];
}

const BuiltInSkillPackDocumentV1Schema = strictObject({
  version: z.literal(1),
  skills: z.array(SkillDocumentSchema).min(1).max(100_000),
});
...
function invalidArtifact(message: string, cause?: unknown): StorageError {
  return new StorageError('SCHEMA_ARTIFACT_INVALID', message, { cause });
}
...
function parseManifest(bytes: Uint8Array): ManifestV1 {
  ...
  for (const artifact of ['project-v1.sql', 'schema-bindings.v1.json', 'built-in-skills.v1.json'])
...
function parseBuiltInSkills(bytes: Uint8Array): BuiltInSkillPackDocumentV1 {
  let parsed: unknown;
  try {
    parsed = JSON.parse(Buffer.from(bytes).toString('utf8'));
  } catch (cause) {
  ...
  let document: BuiltInSkillPackDocumentV1;
  try {
    document = parseCanonical(BuiltInSkillPackDocumentV1Schema, parsed);
  } catch (cause) {
  ...
  for (const skill of document.skills) {
    ...
    if (
      ...
      skill.content.includes('legacy-skill-content') ||
...
export async function loadCanonicalBuiltInSkillPack(
  ...
  return Object.freeze({
    version: 1,
    artifactSha256,
    skills: parseBuiltInSkills(skillsBytes).skills,
  });
}
