...
function prepareAssessment(
  ...
  return withImmediateTransaction(database, () => {
    ...
    if (currentKey.fingerprint !== key.fingerprint) {
      throw new StorageError('REVISION_CONFLICT', 'Result Assessment input changed');
    }
    ...
    if (canonicalJson(request) !== canonicalJson(expectedRequest)) {
      throw new StorageError('REVISION_CONFLICT', 'Result Assessment request changed');
    }
    resolveSubjects(database, run, request);
    ...
    if (!isLocal(request)) {
      ...
      if (canonicalJson(profile) !== canonicalJson(expectedProfile) || cost === null) {
        throw new StorageError('REVISION_CONFLICT', 'Result Assessment provider changed');
      }
...
export function createResultAssessmentsAuthority(
  ...
  return Object.freeze({
    ...
    async executeLocal(inputValue: ContinueResultAssessmentInput, contextValue: CommandContext) {
