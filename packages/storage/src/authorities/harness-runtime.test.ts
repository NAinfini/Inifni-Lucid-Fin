...
import { createAes256GcmPrivateRecoveryCodec, openStore } from '@lucid-fin/storage';
...
function getRun(data: DataAccess, runId: string, suffix: string) {
...
async function generatedDecisionCandidate(
  ...
  projectId: string,
  runId: string,
  context: Parameters<DataAccess['harness']['prepareModelBoundary']>[1],
) {
...
function prepareModelAttempt(
  data: DataAccess,
  input: Parameters<DataAccess['harness']['prepareModelBoundary']>[0],
  context: Parameters<DataAccess['harness']['prepareModelBoundary']>[1],
) {
...
function rowCount(store: Store, table: string): number {
...
function queueHarnessFollowup(data: DataAccess, runId: string, suffix: string) {
...
function settleInitialChildBoundary(data: DataAccess, runId: string, suffix: string) {
...
interface InvalidAgentSendSetup {
  ...
  readonly senderContext?: Parameters<DataAccess['harness']['prepareModelBoundary']>[1];
...
describe('Harness persistence authority', () => {
  ...
  it('cold-reopens and atomically closes one committed undispatched safe R call', async () => {
    ...
    try {
      ...
      fixture.store.close();
      reopened = await openStore(fixture.databasePath);
