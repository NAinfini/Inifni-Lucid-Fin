...
function requireProjectRevision(
  ...
  if (row.revision !== expectedRevision) {
    throw new StorageError('REVISION_CONFLICT', `Project ${projectId} revision changed`);
  }
}
...
function requireProductionRef(
  ...
  if (object.revision !== ref.revision || object.contentHash !== ref.contentHash) {
    throw new StorageError('REVISION_CONFLICT', `Production object ${object.id} changed`);
  }
  return object;
}
...
function corruptProductionMutation(message: string): StorageError {
...
function assertProductionPlanCurrent(
  ...
  if (planned.command !== null) {
    ...
    try {
      ...
      throw new StorageError('REVISION_CONFLICT', 'Production mutation changed before commit', {
        cause,
      });
    }
  ...
  for (const before of planned.watchedProduction) {
    ...
    if (canonicalJson(current) !== canonicalJson(before)) {
      throw new StorageError('REVISION_CONFLICT', `Production ${before.id} changed`);
    }
  }
...
function decodeProductionCursor(cursor: string): z.output<typeof ProductionCursorSchema> {
  try {
    ...
    throw new StorageError('INVALID_REQUEST', 'Production query cursor is invalid', {
      cause,
    });
  }
}
...
function queryProduction(
  ...
  if (
    ...
    throw new StorageError('NOT_FOUND', `Project ${request.input.projectId} was not found`);
  }
  ...
  if (cursor !== null && cursor.filterHash !== filterHash) {
    throw new StorageError('INVALID_REQUEST', 'Production cursor belongs to another query');
  }
  const idClause =
...
function decodeProductionToolCursor(cursor: string | null) {
  ...
  try {
    ...
    throw new StorageError('INVALID_REQUEST', 'Production tool query cursor is invalid', {
      cause,
    });
  }
}
...
export function createProductionAuthority(
  ...
  const authority: ProductionAuthority = {
    ...
    query(request) {
      return queryProduction(getStoreDatabase(store), exactRequest(request, 'production.query'));
    },
    queryTool(projectId, input) {
      return queryProductionTool(getStoreDatabase(store), projectId, input);
    },
  };
