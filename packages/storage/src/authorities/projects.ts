...
function requestFor<Method extends WireRequestV1['method']>(
  ...
  if (request.method !== method) {
    throw new StorageError('INVALID_REQUEST', `Expected ${method} Wire request`);
  }
...
function successFor<Method extends WireSuccessV1['method']>(
  ...
  if (response.kind !== 'success' || response.method !== request.method) {
    throw new StorageError('CORRUPT_DATA', 'Constructed Wire response is invalid');
  }
...
function projectFromRow(row: ProjectRow): Project {
  ...
  try {
    ...
    throw new StorageError('CORRUPT_DATA', `Project ${row.id} is invalid`, { cause });
  }
...
export function getSettings(database: DatabaseSync, projectId: string): ProjectSettings {
  ...
  try {
    ...
    if (cause instanceof StorageError) throw cause;
    throw new StorageError('CORRUPT_DATA', `Project settings ${projectId} are invalid`, {
      cause,
    });
  }
