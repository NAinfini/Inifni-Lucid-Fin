...
function decodeCursor(cursor: string): z.output<typeof ProjectMediaListCursorSchema> {
  try {
    ...
    throw new StorageError('INVALID_REQUEST', 'Project Media cursor is invalid', { cause });
  }
}
...
function requireProjectRevision(
  ...
  if (project.revision !== expectedRevision) {
    throw new StorageError('REVISION_CONFLICT', `Project ${projectId} revision does not match`);
  }
}
...
function requirePublicLinkContext(context: CommandContext): void {
  if (
...
export function linkProjectMediaInTransaction(
  ...
  if (before.lifecycle !== 'active') {
    throw new StorageError('INVALID_REQUEST', `Project Media reference ${before.id} is not active`);
  }
  ...
  if (target.lifecycle !== 'active') {
    throw new StorageError('INVALID_REQUEST', `Production object ${target.id} is not active`);
  }
  if (
    target.revision !== input.target.revision ||
    target.contentHash !== input.target.contentHash
  ) {
    throw new StorageError('REVISION_CONFLICT', `Production object ${target.id} does not match`);
  }
  ...
  if (input.mode === 'link') {
    ...
    if (Number(removed.changes) !== 1) {
      throw new StorageError('REVISION_CONFLICT', 'Project Media link changed concurrently');
    }
  }
...
export function createProjectMediaAuthority(
  ...
  const authority: ProjectMediaAuthority = {
    attach(request, context) {
      ...
      return attachProjectMedia(getStoreDatabase(store), environment, parsed, context);
    },
    detach(request, context) {
      const parsed = exactRequest(request, 'media.project.detach');
      return detachProjectMedia(getStoreDatabase(store), environment, parsed, context);
    },
    link(request, context) {
      const parsed = exactRequest(request, 'media.project.link');
      return linkProjectMedia(getStoreDatabase(store), environment, parsed, context);
    },
    get(projectMediaRefId) {
      return loadProjectMediaRef(getStoreDatabase(store), projectMediaRefId);
    },
    list(request) {
      ...
      if (
        ...
        throw new StorageError('NOT_FOUND', `Project was not found: ${parsed.input.projectId}`);
      }
      ...
      if (cursor !== null && cursor.filterHash !== filterHash) {
        throw new StorageError('INVALID_REQUEST', 'Project Media cursor belongs to another query');
      }
      const searchJoin =
