...
function assertExpectedRun(run: Run, expectedRevision: number): void {
  if (run.revision !== expectedRevision) {
    throw new StorageError('REVISION_CONFLICT', `Run ${run.id} revision does not match`);
  }
  if (RunTerminalStateSchema.safeParse(run.status).success) {
    throw new StorageError('INVALID_REQUEST', `Terminal Run ${run.id} cannot be changed`);
  }
}

function journalContext(contextValue: CommandContext): CommandContext {
  return parseCanonical(CommandContextSchema, contextValue);
}

function eventId(environment: StorageEnvironment): string {
...
function appendInboxEvents(
  ...
  if (appended.length !== events.length)
    throw new StorageError('CORRUPT_DATA', 'Inbox event was not appended');
  return appended;
}
...
export function terminalizeRunInTransaction(
  ...
  if (head === undefined) throw new StorageError('CORRUPT_DATA', 'Run event batch is empty');
...
function listPublicEvents(
  ...
  if (cursorValue !== null) {
    try {
      ...
      throw new StorageError('INVALID_REQUEST', 'Run event cursor is invalid', { cause });
    }
    if (cursor.filterHash !== filterHash) {
      throw new StorageError('INVALID_REQUEST', 'Run event cursor belongs to another query');
    }
  }
...
function enqueueFollowupInTransaction(
  ...
  if (event === undefined) throw new StorageError('CORRUPT_DATA', 'Follow-up event missing');
  ...
  return executeWireMutation(
    ...
    () => {
      ...
      if (request.input.expectedRevision > before.revision) {
        throw new StorageError('REVISION_CONFLICT', `Run ${before.id} revision does not match`);
      }
...
function transitionInbox(
  ...
  return withImmediateTransaction(database, () => {
    ...
    if (
      ...
      throw new StorageError('INVALID_REQUEST', 'Inbox messages must advance in FIFO order');
    }
    const state =
    ...
    try {
      ...
      throw new StorageError('INVALID_REQUEST', `Inbox transition is invalid`, { cause });
    }
    ...
    const head = events.at(-1);
    if (head === undefined) throw new StorageError('CORRUPT_DATA', 'Inbox event is missing');
    advanceRunJournalHead(database, run, {
...
function startActivation(
  ...
  return withImmediateTransaction(database, () => {
    ...
    if (existing.some(({ state }) => state === 'active')) {
      throw new StorageError('INVALID_REQUEST', `Run ${run.id} already has an active Activation`);
    }
    if (run.parentRunId === null) {
      ...
      if (competing !== undefined) {
        throw new StorageError('INVALID_REQUEST', `Chat ${run.chatId} has an active root Run`);
      }
    }
    ...
    if (event === undefined) throw new StorageError('CORRUPT_DATA', 'Activation event missing');
...
function endActivation(
  ...
  return withImmediateTransaction(database, () => {
    ...
    if (before.state !== 'active') {
      throw new StorageError('INVALID_REQUEST', `Activation ${input.activationNumber} ended`);
    }
    const [event] = appendRunEventBatch(database, {
      runId: run.id,
      commandId: input.commandId,
...
export interface RunsAuthority {
  ...
  readonly terminalize: (input: HostRunTerminalizeInput, context: CommandContext) => Run;
  readonly spawnChild: (
  ...
  readonly startActivation: (input: ActivationStartInput, context: CommandContext) => RunActivation;
  readonly endActivation: (input: ActivationEndInput, context: CommandContext) => RunActivation;
}
...
export function createRunsAuthority(
  ...
  const authority: RunsAuthority = {
    get(request) {
      return runGet(getStoreDatabase(store), exactRequest(request, 'run.get'));
    },
    isSchedulingAllowed(runId) {
      return isRunSchedulingAllowed(getStoreDatabase(store), runId);
    },
    listPublicEvents(request) {
      return listPublicEvents(getStoreDatabase(store), exactRequest(request, 'run.events.list'));
    },
    ...
    terminalize(input, context) {
      return terminalizeRun(getStoreDatabase(store), environment, input, context);
    },
    ...
    transitionInbox(input, context) {
      return transitionInbox(getStoreDatabase(store), environment, input, context);
    },
    listActivations(runId) {
      return loadRunActivations(getStoreDatabase(store), runId);
    },
    startActivation(input, context) {
      return startActivation(getStoreDatabase(store), environment, input, context);
    },
    endActivation(input, context) {
      return endActivation(getStoreDatabase(store), environment, input, context);
    },
  };
