...
function projectEventWatermark(database: ReturnType<typeof getStoreDatabase>, projectId: string) {
  return (
    database
      .prepare(
...
function acceptanceCounts(database: ReturnType<typeof getStoreDatabase>) {
...
describe('message.send root Run acceptance', () => {
  ...
  it('reopens and returns the identical receipt without duplicating any acceptance row', async () => {
    ...
    try {
      ...
      expect(acceptanceCounts(getStoreDatabase(reopened))).toEqual(before);
    } finally {
      reopened.close();
    }
  });
