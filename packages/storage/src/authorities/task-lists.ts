...
function requireActiveList(database: DatabaseSync, runId: string, expectedRevision: number) {
  ...
  if (taskList === null) {
    throw new StorageError('NOT_FOUND', `Run ${runId} has no TaskList`);
  }
  if (taskList.revision !== expectedRevision) {
    throw new StorageError('REVISION_CONFLICT', `TaskList ${taskList.id} revision does not match`);
  }
  if (taskList.state !== 'active') {
    throw new StorageError('INVALID_REQUEST', `TaskList ${taskList.id} is terminal`);
  }
  return taskList;
}
...
function assertParent(items: readonly TaskItem[], parentTaskId: string | null): void {
  if (parentTaskId !== null && !items.some(({ id }) => id === parentTaskId)) {
    throw new StorageError('INVALID_REQUEST', `Task parent was not found: ${parentTaskId}`);
  }
}
...
function assertLinkedChild(database: DatabaseSync, owner: Run, childRunId: string): void {
  ...
  throw new StorageError('INVALID_REQUEST', `Run ${childRunId} is not a descendant of ${owner.id}`);
}
...
function appendTaskListEvent(
  ...
  if (event === undefined) {
    throw new StorageError('CORRUPT_DATA', 'TaskList event was not appended');
  }
...
export function manageTaskListInTransaction(
  ...
  if (
    ...
    throw new StorageError('REVISION_CONFLICT', `Run ${run.id} observation does not match`);
  }
  ...
  if (input.action === 'create') {
    ...
    if (loadTaskList(database, run.id) !== null) {
      throw new StorageError('INVALID_REQUEST', `Run ${run.id} already has a TaskList`);
    }
  ...
  switch (input.action) {
    ...
    case 'add': {
      ...
      if (input.order > siblings.length) {
        throw new StorageError('INVALID_REQUEST', 'Task insertion order is out of range');
      }
    ...
    case 'update': {
      ...
      if (index < 0) {
        throw new StorageError('NOT_FOUND', `Task item was not found: ${input.taskId}`);
      }
      ...
      if (input.state !== null && input.state !== item.state) {
        try {
          ...
          throw new StorageError('INVALID_REQUEST', 'Task item state transition is invalid', {
            cause,
          });
        }
    ...
    case 'remove': {
      ...
      if (items.some(({ parentItemId }) => parentItemId === removed.id)) {
        throw new StorageError('INVALID_REQUEST', 'Only a leaf Task item may be removed');
      }
      items = items
...
export function createTaskListsAuthority(
  ...
  const authority: TaskListsAuthority = {
    ...
    manage(runId, input, options) {
      return manageTaskList(getStoreDatabase(store), environment, runId, input, options);
    },
  };
