...
function assertRunCas(run: Run, expectedRevision: number, expectedRunHash: string): void {
  if (run.revision !== expectedRevision || run.contentHash !== expectedRunHash) {
    throw new StorageError('REVISION_CONFLICT', `Run ${run.id} CAS does not match`);
  }
...
function appendStage(
  ...
  if (event?.visibility !== 'model_surface') {
    throw new StorageError('CORRUPT_DATA', 'Compaction stage event was not appended');
  }
...
function assertCommittedRange(
  ...
  if (row.count !== sourceEventTo - sourceEventFrom + 1) {
    throw new StorageError('CORRUPT_DATA', 'Compaction source range is not contiguous');
  }
}
...
function startCompaction(
  ...
  const context = parseStageInput(CommandContextSchema, contextValue, 'started');
  const semantics = {
  ...
  return withImmediateTransaction(database, () => {
    ...
    if (activation?.activation.activationNumber !== input.activationNumber) {
      throw new StorageError('REVISION_CONFLICT', 'Active Activation number does not match');
    }
...
function deriveCompactionView(
  ...
  const context = parseStageInput(CommandContextSchema, contextValue, 'view_derived');
...
function completeCompaction(
  ...
  const context = parseStageInput(CommandContextSchema, contextValue, 'completed');
...
function interruptCompaction(
  ...
  const context = parseStageInput(CommandContextSchema, contextValue, 'interrupted');
...
export interface CompactionsAuthority {
  readonly start: (input: CompactionStartInput, context: CommandContext) => CompactionStageResult;
  readonly deriveView: (
...
export function createCompactionsAuthority(
  ...
  const authority: CompactionsAuthority = {
    start(input, context) {
      return startCompaction(getStoreDatabase(store), environment, input, context);
    },
    deriveView(input, context) {
      return deriveCompactionView(getStoreDatabase(store), environment, input, context);
    },
    complete(input, context) {
      return completeCompaction(getStoreDatabase(store), environment, input, context);
    },
    interrupt(input, context) {
      return interruptCompaction(getStoreDatabase(store), environment, input, context);
    },
