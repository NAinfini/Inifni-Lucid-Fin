...
import { canonicalJson, type GlobalMediaAsset, type GlobalMediaFolder } from '@lucid-fin/contracts';
import type { CommandContext } from '../internal/command.js';
...
function deterministicEnvironment(): StorageEnvironment {
...
describe('Global Media authority', () => {
  ...
  it('imports verified bytes, reuses receipts before capability I/O, lists, and removes only the asset', async () => {
    ...
    try {
      ...
      expect(
        getStoreDatabase(store).prepare('SELECT COUNT(*) AS count FROM media_blobs').get(),
      ).toEqual({ count: 1 });
