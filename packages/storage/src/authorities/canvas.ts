...
function placementIndex(canvas: CanvasDocument, placementId: string): number {
  ...
  if (index < 0) {
    throw new StorageError('NOT_FOUND', `Canvas placement ${placementId} was not found`);
  }
  return index;
}
...
function invalidCanvasMutation(message: string): StorageError {
  return new StorageError('INVALID_REQUEST', message);
}

function corruptCanvasMutation(message: string): StorageError {
...
function mutateCanvasCore(
  ...
  switch (command.action) {
    ...
    case 'restore_view': {
      ...
      if (view === undefined) {
        throw new StorageError('NOT_FOUND', `Canvas saved view ${command.viewId} was not found`);
      }
...
function requireGroupRevision(
  ...
  if (group === undefined)
    throw new StorageError('NOT_FOUND', `Canvas group ${groupId} was not found`);
  if (group.revision !== expectedRevision) {
    throw new StorageError('REVISION_CONFLICT', `Canvas group ${groupId} revision changed`);
  }
  return group;
}
...
function requireEdgeRevision(
  ...
  if (edge === undefined)
    throw new StorageError('NOT_FOUND', `Canvas edge ${edgeId} was not found`);
  if (edge.revision !== expectedRevision) {
    throw new StorageError('REVISION_CONFLICT', `Canvas edge ${edgeId} revision changed`);
  }
  return edge;
}
...
function requireViewRevision(
  ...
  if (view === undefined) {
    throw new StorageError('NOT_FOUND', `Canvas saved view ${viewId} was not found`);
  }
  if (view.revision !== expectedRevision) {
    throw new StorageError('REVISION_CONFLICT', `Canvas saved view ${viewId} revision changed`);
  }
  return view;
}
...
function assertCanvasMutationPlanCurrent(
  ...
  try {
    ...
    if (cause instanceof StorageError && cause.code === 'CORRUPT_DATA') throw cause;
    throw new StorageError('REVISION_CONFLICT', 'Canvas mutation changed before commit', {
      cause,
    });
  }
...
function decodeCanvasToolCursor(cursor: string | null) {
  ...
  try {
    ...
    throw new StorageError('INVALID_REQUEST', 'Canvas tool query cursor is invalid', {
      cause,
    });
  }
}
...
function queryCanvasTool(
  ...
  if (
    ...
    throw new StorageError('INVALID_REQUEST', 'Canvas tool query cursor is stale');
  }
...
function applyCanvas(
  ...
  return executeWireMutation(database, request, context, now, () => {
    ...
    const changed = mutateCanvasCore(
      database,
      before,
      coreCommandFromWire(environment, request.input.command),
      now,
    );
    if (changed === null) {
...
export function createCanvasAuthority(
  ...
  const authority: CanvasAuthority = {
    ...
    queryTool(projectId, input) {
      return queryCanvasTool(getStoreDatabase(store), projectId, input);
    },
