...
import { canonicalJson, type SkillDocument } from '@lucid-fin/contracts';
...
describe('host built-in Skill registration', () => {
    ...
    try {
      ...
      expect(
        database
          ...
          .all(),
      ).toEqual([{ provenance: 'built_in', trust: 'trusted', count: 287 }]);
      expect(
  ...
  it('uses explicit effective versions while quarantining all unreviewed content', () => {
    ...
    try {
      ...
      const override = {
        ...skill('skill.shared', 'user override', 'installed', 'unreviewed'),
        version: '1.0.1',
      };
