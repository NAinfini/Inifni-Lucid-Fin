...
export function replaceCanvas(
  ...
  if (Number(updated.changes) !== 1) {
    throw new StorageError('REVISION_CONFLICT', `Canvas ${before.id} changed concurrently`);
  }
  for (const table of [
...
function loadCanvasFromRow(database: DatabaseSync, row: CanvasRow): CanvasDocument {
  ...
  try {
    ...
    throw new StorageError('CORRUPT_DATA', `Canvas ${row.id} is invalid`, { cause });
  }
  if (hashContentObject(canvas) !== canvas.contentHash) {
    throw new StorageError('CORRUPT_DATA', `Canvas ${row.id} content hash does not match`);
  }
  return canvas;
}
