...
import { executeWireMutation, readWireMutationReceipt, type CommandContext } from './command.js';
...
function validateProvider(database: DatabaseSync, seed: MessageSendAcceptanceSeed): void {
  ...
  if (profile === undefined) {
    throw new StorageError('NOT_FOUND', `Provider profile was not found: ${seed.model.providerId}`);
  }
...
export function findActiveRootRun(database: DatabaseSync, chatId: string): Run | null {
  ...
  if (active.length > 1) {
    throw new StorageError('CORRUPT_DATA', `Chat ${chatId} has multiple active root Runs`);
  }
...
function captureMemoryContext(
  ...
  if (missing !== undefined) {
    throw new StorageError('NOT_FOUND', `Project Memory entry was not found: ${missing}`);
  }
  return {
    state: 'ready',
    derivationVersion: head.index.derivationVersion,
...
function createAcceptanceIds(environment: StorageEnvironment, acceptedAt: string): AcceptanceIds {
  return {
...
function acceptRootRunInTransactionCore(
  ...
  if (!database.isTransaction) {
    throw new StorageError('INVALID_REQUEST', 'Root Run acceptance requires an active transaction');
  }
  ...
  if (message.role !== 'user') {
    throw new StorageError('CORRUPT_DATA', 'Root acceptance appended a non-user Message');
  }
  ...
  try {
    ...
    throw new StorageError('CORRUPT_DATA', 'Run acceptance snapshot is inconsistent', {
      cause,
    });
  }
...
function availablePublicPayload(event: ReturnType<typeof loadRunEvents>[number]) {
  if (event.visibility !== 'public' || event.payloadState.state !== 'available') {
    throw new StorageError('CORRUPT_DATA', 'Recovery source journal tail is unavailable');
  }
...
function assertExpectedRecoverySource(
  ...
  if (
    ...
    throw new StorageError('REVISION_CONFLICT', `Recovery source Run ${source.id} changed`);
  }
  if (
  ...
  if (
    ...
    throw new StorageError('CORRUPT_DATA', `Recovery source Run ${source.id} head differs`);
  }
  ...
  if (
    ...
    throw new StorageError('CORRUPT_DATA', `Recovery source Run ${source.id} tail is invalid`);
  }
...
function replayCrashRetry(
  ...
  if (source.acceptedSource.kind !== 'message') {
    throw new StorageError('CORRUPT_DATA', `Run ${source.id} retry source is not a Message`);
  }
  const rows = database
  ...
  if (rows.length !== 1) {
    throw new StorageError('CORRUPT_DATA', `Run ${source.id} has duplicate crash retries`);
  }
...
export function acceptCrashRetryRootRun(
  ...
  return withImmediateTransaction(database, () => {
    ...
    if (source.acceptedSource.kind !== 'message') {
      throw new StorageError('CORRUPT_DATA', `Run ${source.id} source is not a Message`);
    }
    ...
    if (
      ...
      throw new StorageError('CORRUPT_DATA', `Run ${source.id} source Inbox is invalid`);
    }
    ...
    if (
      ...
      throw new StorageError('CORRUPT_DATA', `Run ${source.id} objective Message changed`);
    }
