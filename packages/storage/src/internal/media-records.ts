...
export function loadMediaBlob(database: DatabaseSync, hash: string): MediaBlob {
  ...
  if (blob === undefined) throw new StorageError('NOT_FOUND', `Media Blob was not found: ${hash}`);
  return blob;
}
...
export function loadGlobalMediaFolder(database: DatabaseSync, id: string): GlobalMediaFolder {
  ...
  if (folder === undefined) {
    throw new StorageError('NOT_FOUND', `Global Media Folder was not found: ${id}`);
  }
  return folder;
}
...
export function loadGlobalMediaAsset(database: DatabaseSync, id: string): GlobalMediaAsset {
  ...
  if (asset === undefined) {
    throw new StorageError('NOT_FOUND', `Global Media Asset was not found: ${id}`);
  }
  return asset;
}
...
export function loadProjectMediaRecord(
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Project Media reference was not found: ${id}`);
  }
