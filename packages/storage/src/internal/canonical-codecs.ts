...
function decodeJson(label: string, json: string): unknown {
  try {
    ...
    throw new StorageError('CORRUPT_DATA', `${label} is not valid JSON`, { cause });
  }
}
...
export function decodeWireSuccess(json: string): WireSuccessV1 {
  try {
    ...
    throw new StorageError('CORRUPT_DATA', 'Wire success receipt is invalid', { cause });
  }
}
...
export function decodeProjectEventPayload(json: string): ProjectEventPayload {
  try {
    ...
    if (cause instanceof StorageError) throw cause;
    throw new StorageError('CORRUPT_DATA', 'ProjectEvent payload is invalid', { cause });
  }
}
...
function decodePersisted<Output>(
  ...
  try {
    ...
    if (cause instanceof StorageError) throw cause;
    throw new StorageError('CORRUPT_DATA', `${label} is invalid`, { cause });
  }
}
...
export function decodeCanonicalRecord<Schema extends z.ZodType>(
  ...
  if (canonicalJson(value) !== json) {
    throw new StorageError('CORRUPT_DATA', `${label} is not canonical`);
  }
  return value;
}
