...
export function insertAcceptedRunSnapshot(
  ...
  if (!database.isTransaction) {
    throw new StorageError('INVALID_REQUEST', 'Run snapshot insert requires a transaction');
  }
  if (
    run.revision !== 0 ||
    run.status !== 'accepted' ||
    ...
    throw new StorageError('CORRUPT_DATA', 'Accepted Run snapshot is not initial');
  }
  try {
    assertRunContextManifest(run, manifest, catalog);
  } catch (cause) {
    throw new StorageError('CORRUPT_DATA', 'Accepted Run snapshot is inconsistent', {
      cause,
    });
  }
  if (
    ...
    throw new StorageError('CORRUPT_DATA', 'Accepted Run snapshot hash does not match');
  }
