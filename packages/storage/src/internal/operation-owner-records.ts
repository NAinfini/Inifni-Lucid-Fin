...
export function loadGenerationOwner(database: DatabaseSync, id: string): OperationOwnerRecord {
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Generation Attempt was not found: ${id}`);
  }
...
function loadMediaDerivationOwner(database: DatabaseSync, id: string): OperationOwnerRecord {
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Media Derivation Attempt was not found: ${id}`);
  }
...
export function transitionGenerationOwner(
  ...
  if (!database.isTransaction) {
    throw new StorageError('INVALID_REQUEST', 'Generation owner transition requires a transaction');
  }
  if (before.authority !== 'generation_attempt' || before.view.authority !== 'generation_attempt') {
    throw new StorageError('INVALID_REQUEST', 'Operation owner is not a Generation Attempt');
  }
  if (
    ...
    throw new StorageError('INVALID_REQUEST', 'Generation provider receipt is immutable');
  }
  if (
    before.view.usage !== null &&
    canonicalJson(before.view.usage) !== canonicalJson(input.usage)
  ) {
    throw new StorageError('INVALID_REQUEST', 'Generation provider usage is immutable');
  }
  const withoutHash = {
    ...before.view,
  ...
  try {
    ...
    throw new StorageError('INVALID_REQUEST', 'Generation transition is invalid', {
      cause,
    });
  }
...
export function transitionResultAssessmentOwner(
  ...
  if (
    ...
    throw new StorageError('INVALID_REQUEST', 'Operation owner is not a Result Assessment Attempt');
  }
  if (
    before.view.receipt !== null &&
    (input.receipt === null ||
    ...
    throw new StorageError('INVALID_REQUEST', 'Result Assessment provider receipt is immutable');
  }
  if (
    before.view.usage !== null &&
    canonicalJson(before.view.usage) !== canonicalJson(input.usage)
  ) {
    throw new StorageError('INVALID_REQUEST', 'Result Assessment provider usage is immutable');
  }
  if (
    before.view.assessment !== null &&
    canonicalJson(before.view.assessment) !== canonicalJson(input.assessment)
  ) {
    throw new StorageError('INVALID_REQUEST', 'Final Assessment is immutable');
  }
  const withoutHash = {
    ...before.view,
    revision: before.view.revision + 1,
  ...
  try {
    ...
    throw new StorageError('INVALID_REQUEST', 'Result Assessment transition is invalid', {
      cause,
    });
  }
  const update = database
...
export function transitionMediaDerivationOwner(
  ...
  if (before.authority !== 'media_derivation_attempt') {
    throw new StorageError('INVALID_REQUEST', 'Operation owner is not a Media Derivation');
  }
  if (before.view.authority !== 'media_derivation_attempt') {
    throw new StorageError('INVALID_REQUEST', 'Media Derivation owner view is invalid');
  }
  ...
  if (
    ...
    throw new StorageError('INVALID_REQUEST', 'Media Derivation provider receipt is immutable');
  }
  if (
    before.view.usage !== null &&
    canonicalJson(before.view.usage) !== canonicalJson(input.usage)
  ) {
    throw new StorageError('INVALID_REQUEST', 'Media Derivation provider usage is immutable');
  }
  const withoutHash = {
    ...before.view,
    revision: before.view.revision + 1,
  ...
  try {
    ...
    throw new StorageError('INVALID_REQUEST', 'Media Derivation transition is invalid', {
      cause,
    });
  }
  const update = database
...
export function transitionLocalDeliveryOwner(
  ...
  if (before.authority !== 'review_cut_attempt' && before.authority !== 'delivery_export') {
    throw new StorageError('INVALID_REQUEST', 'Operation owner is not local Delivery work');
  }
  if (before.authority === 'review_cut_attempt' && input.outputContentHash !== null) {
    throw new StorageError('INVALID_REQUEST', 'Review Cut cannot store an export hash');
  }
  ...
  try {
    ...
    throw new StorageError('INVALID_REQUEST', 'Local Delivery transition is invalid', {
      cause,
    });
  }
  const table =
...
export function claimLocalDeliveryOwner(
  ...
  if (!database.isTransaction) {
    throw new StorageError('INVALID_REQUEST', 'Local Delivery owner claim requires a transaction');
  }
  if (before.authority !== 'review_cut_attempt' && before.authority !== 'delivery_export') {
    throw new StorageError('INVALID_REQUEST', 'Operation owner is not local Delivery work');
  }
...
function loadAssessmentOwner(database: DatabaseSync, id: string): OperationOwnerRecord {
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Result Assessment Attempt was not found: ${id}`);
  }
...
function loadReviewCutOwner(database: DatabaseSync, id: string): OperationOwnerRecord {
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Review Cut Attempt was not found: ${id}`);
  }
  if (
...
function loadDeliveryExportOwner(database: DatabaseSync, id: string): OperationOwnerRecord {
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Delivery Export was not found: ${id}`);
  }
  if (
    row.manifest_project_id !== row.project_id ||
    row.stored_manifest_revision !== row.delivery_manifest_revision ||
...
export function loadDeliveryManifest(
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Delivery Manifest was not found: ${manifestId}`);
  }
...
export function loadGeneratedResultRecord(
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Generated Result was not found: ${resultId}`);
  }
...
export function requestOperationOwnerCancellation(
  ...
  if (!database.isTransaction) {
    throw new StorageError('INVALID_REQUEST', 'Operation owner update requires a transaction');
  }
  if (before.view.cancelRequested) {
    throw new StorageError('INVALID_REQUEST', `Operation ${before.view.id} is already cancelled`);
  }
  const withoutHash = {
    ...before.view,
    revision: before.view.revision + 1,
