import type { ProviderUsage, ResourceAmount } from '@lucid-fin/contracts';
...
function corrupt(label: string, message: string): StorageError {
  return new StorageError('CORRUPT_DATA', `${label} ${message}`);
}
