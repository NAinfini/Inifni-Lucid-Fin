...
export function loadModelAttemptRecord(
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Model Attempt was not found: ${attemptId}`);
  }
...
export function insertPreparedModelAttempt(
  ...
  if (!database.isTransaction) {
    throw new StorageError('INVALID_REQUEST', 'Model Attempt insert requires a transaction');
  }
  ...
  if (
    ...
    throw new StorageError('INVALID_REQUEST', 'A new Model Attempt must be prepared');
  }
...
export function markModelAttemptRecordRunning(
  ...
  if (!database.isTransaction) {
    throw new StorageError('INVALID_REQUEST', 'Model Attempt update requires a transaction');
  }
  const attemptId = parseCanonical(EntityIdSchema, attemptIdValue);
  ...
  try {
    ...
    throw new StorageError('INVALID_REQUEST', `Model Attempt ${attemptId} cannot run`, {
      cause,
    });
  }
  ...
  if (Number(update.changes) !== 1) {
    throw new StorageError('REVISION_CONFLICT', `Model Attempt ${attemptId} changed`);
  }
...
function responseState(response: DurableCanonicalModelResponseV1): ModelAttemptRecordV1['state'] {
  ...
  if (terminal.type !== 'model_failed') {
    throw new StorageError('INVALID_REQUEST', 'Model response has no terminal event');
  }
...
export function settleModelAttemptRecord(
  ...
  if (!database.isTransaction) {
    throw new StorageError('INVALID_REQUEST', 'Model Attempt settlement requires a transaction');
  }
  const attemptId = parseCanonical(EntityIdSchema, attemptIdValue);
  const requestHash = parseCanonical(Sha256Schema, requestHashValue);
  ...
  try {
    ...
    throw new StorageError('INVALID_REQUEST', `Model Attempt ${attemptId} cannot settle`, {
      cause,
    });
  }
