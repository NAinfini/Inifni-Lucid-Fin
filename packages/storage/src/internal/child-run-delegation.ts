...
function conflict(message: string, cause?: unknown): StorageError {
...
function deriveCatalog(
  ...
  const capabilityIndex = toolIds.map((id) => {
    ...
    if (entry === undefined) {
      throw new StorageError('CORRUPT_DATA', `Parent catalog index is missing ${id}`);
    }
    return entry;
  });
  const withoutHash = {
  ...
  try {
    ...
    throw new StorageError('CORRUPT_DATA', 'Derived child catalog is not a parent subset', {
      cause,
    });
  }
  if (canonicalJson(catalog.skills) !== canonicalJson(parent.skills)) {
    throw new StorageError('CORRUPT_DATA', 'Derived child catalog changed parent skills');
  }
  return catalog;
}
...
function createDelegatedChildInTransaction(
  ...
  if (event === undefined) {
    throw new StorageError('CORRUPT_DATA', 'Child delegation event was not appended');
  }
  ...
  try {
    ...
    throw new StorageError('CORRUPT_DATA', 'Child delegation snapshot is inconsistent', {
      cause,
    });
  }
...
export function delegateChildRunInTransaction(
  ...
  if (
    ...
    throw new StorageError('REVISION_CONFLICT', 'Child delegation parent CAS baseline changed');
  }
  ...
  if (hashContentObject(observed) !== transactionInput.observedParent.contentHash) {
    throw new StorageError('REVISION_CONFLICT', 'Child delegation observed parent changed');
  }
...
export function delegateToolProgramChildRunInTransaction(
  ...
  if (
    ...
    throw new StorageError('REVISION_CONFLICT', 'Tool Program child parent CAS baseline changed');
  }
  const observed = parseCanonical(RunSchema, {
    ...parent,
  ...
  if (hashContentObject(observed) !== observedParent.contentHash) {
    throw new StorageError('REVISION_CONFLICT', 'Tool Program observed parent changed');
  }
  if (RunTerminalStateSchema.safeParse(parent.status).success) {
