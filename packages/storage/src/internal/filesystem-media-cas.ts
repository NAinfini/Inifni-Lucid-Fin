...
import { CountSchema, Sha256Schema, parseCanonical, strictObject } from '@lucid-fin/contracts';
import {
...
async function inspectOpenFile(handle: FileHandle): Promise<{ byteLength: number; hash: string }> {
  ...
  if (!info.isFile()) {
    throw new StorageError('CORRUPT_DATA', 'Media CAS object is not a regular file');
  }
      ...
      try {
        ...
        for await (const chunk of bytes) {
          if (!(chunk instanceof Uint8Array)) {
            throw new StorageError('INVALID_REQUEST', 'Media byte stream is invalid');
          }
          if (byteLength + chunk.byteLength > expected.byteLength) {
            throw new StorageError('INVALID_REQUEST', 'Media byte stream is longer than expected');
          }
    ...
    async stat(hashInput) {
      ...
      try {
        ...
        try {
          const info = await handle.stat();
          if (!info.isFile()) {
            throw new StorageError('CORRUPT_DATA', `Media CAS object ${hash} is not a file`);
          }
    ...
    async verify(expectedInput) {
      ...
      try {
        ...
        if ((cause as NodeJS.ErrnoException).code === 'ENOENT') {
          throw new StorageError('CORRUPT_DATA', `Media CAS object ${expected.hash} is missing`, {
            cause,
          });
        }
        throw cause;
