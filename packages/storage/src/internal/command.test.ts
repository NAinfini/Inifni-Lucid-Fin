...
import type { WireRequestV1, WireSuccessV1 } from '@lucid-fin/contracts';
...
import { executeWireMutation, type CommandContext } from './command.js';
...
async function openDatabase(): Promise<{ database: DatabaseSync; close(): void }> {
  ...
  return { database: getStoreDatabase(store), close: () => store.close() };
}
...
describe('StorageError', () => {
  it('retains typed codes without exposing internal payloads', () => {
    expect(new StorageError('CORRUPT_DATA', 'bad').code).toBe('CORRUPT_DATA');
  });
});
