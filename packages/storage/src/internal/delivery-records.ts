...
export function loadDeliveryPlanRecord(database: DatabaseSync, planId: string): DeliveryPlan {
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Delivery plan was not found: ${planId}`);
  }
  try {
    const items = (
      database
...
function updateItem(
  ...
  if (
    ...
    throw new StorageError('INVALID_REQUEST', `Delivery item ${before.id} revision is invalid`);
  }
  ...
  if (Number(result.changes) !== 1) {
    throw new StorageError('REVISION_CONFLICT', `Delivery item ${before.id} changed`);
  }
}
...
export function writeDeliveryPlanRecord(
  ...
  if (
    ...
    throw new StorageError('INVALID_REQUEST', 'Delivery owner revision is invalid');
  }
  ...
  if ([...beforeItems.keys()].some((id) => !afterItems.has(id))) {
    throw new StorageError('INVALID_REQUEST', 'Delivery items are historical and cannot vanish');
  }
  ...
  if (itemRowsChanged) {
    ...
    for (const item of after.items) {
      ...
      if (old === undefined) {
        if (item.revision !== 0) {
          throw new StorageError('INVALID_REQUEST', `New Delivery item ${item.id} is invalid`);
        }
  ...
  if (Number(updated.changes) !== 1) {
    throw new StorageError('REVISION_CONFLICT', `Delivery ${before.id} changed`);
  }
}
