...
export function loadRunSnapshots(
  ...
  if (manifestRow === undefined || catalogRow === undefined) {
    throw new StorageError('CORRUPT_DATA', `Run ${run.id} snapshot rows are incomplete`);
  }
  ...
  if (
    ...
    throw new StorageError('CORRUPT_DATA', `Run ${run.id} snapshot hashes do not match`);
  }
  try {
    assertRunContextManifest(run, manifest, catalog);
    assertFrozenSkillRows(database, run.projectId, catalog);
  } catch (cause) {
    throw new StorageError('CORRUPT_DATA', `Run ${run.id} snapshot is inconsistent`, {
      cause,
    });
  }
