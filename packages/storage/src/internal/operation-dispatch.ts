...
export function loadOperationDispatch(
  ...
  if (row === undefined) {
    throw new StorageError('NOT_FOUND', `Operation was not found: ${operationId}`);
  }
...
export function transitionRuntimeDispatchGuard(
  ...
  if (Number(update.changes) !== 1) {
    throw new StorageError('REVISION_CONFLICT', `Dispatch ${dispatch.id} guard changed`);
  }
  return loadOperationDispatch(database, dispatch.id);
}
...
export function settleValidatedRuntimeDispatch(
  ...
  if (Number(update.changes) !== 1) {
    throw new StorageError('REVISION_CONFLICT', `Dispatch ${dispatch.id} changed`);
  }
  return Object.freeze({
    ...dispatch,
    outcome,
...
export function loadBoundOperation(
  ...
  if (
    ...
    throw new StorageError('NOT_FOUND', `Dispatch ${dispatch.id} is not an Operation`);
  }
