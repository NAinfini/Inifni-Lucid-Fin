...
export function insertRunInboxMessage(database: DatabaseSync, inboxValue: RunInboxMessage): void {
  if (!database.isTransaction) {
    throw new StorageError('INVALID_REQUEST', 'Inbox insert requires an active transaction');
  }
...
export function listRunInbox(database: DatabaseSync, runIdValue: string): RunInboxMessage[] {
  ...
  if (messages.some((message, index) => message.sequence !== index + 1)) {
    throw new StorageError('CORRUPT_DATA', `Run ${runId} Inbox sequence is not contiguous`);
  }
  try {
    assertInboxOrdering(messages);
  } catch (cause) {
    throw new StorageError('CORRUPT_DATA', `Run ${runId} Inbox order is invalid`, { cause });
  }
  return messages;
}
...
export function updateRunInboxState(
  ...
  if (Number(update.changes) !== 1) {
    throw new StorageError('REVISION_CONFLICT', `Inbox message ${before.id} changed concurrently`);
  }
