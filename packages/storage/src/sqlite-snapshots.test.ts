...
describe('sqlite-snapshots', () => {
  ...
  it('upsertSession creates a new session', () => {
    ...
    const rows = db.repos.sessions.list().rows;
  ...
  it('upsertSession updates an existing session', () => {
    db.repos.sessions.upsert(SESSION);
    db.repos.sessions.upsert({ ...SESSION, title: 'Updated', updatedAt: 2000 });
    const rows = db.repos.sessions.list().rows;
    expect(rows).toHaveLength(1);
  ...
  it('getSession returns undefined for unknown id', () => {
    expect(db.repos.sessions.get(parseSessionId('nope'))).toBeUndefined();
  });

  it('getSession returns the session by id', () => {
    db.repos.sessions.upsert(SESSION);
    const s = db.repos.sessions.get(parseSessionId('sess-1'));
  ...
  it('deleteSession removes the session', () => {
    db.repos.sessions.upsert(SESSION);
    db.repos.sessions.delete(parseSessionId('sess-1'));
    expect(db.repos.sessions.list().rows).toHaveLength(0);
  });

  it('listSessions returns newest first', () => {
    db.repos.sessions.upsert({ ...SESSION, id: 'a', updatedAt: 1000 });
    db.repos.sessions.upsert({ ...SESSION, id: 'b', updatedAt: 2000 });
    const rows = db.repos.sessions.list().rows;
  ...
  it('listSessions respects limit', () => {
    for (let i = 0; i < 10; i++) {
      db.repos.sessions.upsert({ ...SESSION, id: `s${i}`, updatedAt: i });
    }
    expect(db.repos.sessions.list(5).rows).toHaveLength(5);
  });
  ...
  it('insertSnapshot creates a snapshot', () => {
    ...
    const snaps = db.repos.snapshots.list(parseSessionId('sess-1')).rows;
  ...
  it('listSnapshots returns newest first', () => {
    db.repos.sessions.upsert(SESSION);
    db.repos.snapshots.insert({ id: 'a', sessionId: 'sess-1', label: 'a', trigger: 'auto', data: '{}', createdAt: 1000 });
    db.repos.snapshots.insert({ id: 'b', sessionId: 'sess-1', label: 'b', trigger: 'auto', data: '{}', createdAt: 2000 });
    const snaps = db.repos.snapshots.list(parseSessionId('sess-1')).rows;
  ...
  it('deleteSnapshot removes the snapshot', () => {
    db.repos.sessions.upsert(SESSION);
    db.repos.snapshots.insert({ id: 'snap-1', sessionId: 'sess-1', label: '', trigger: 'auto', data: '{}', createdAt: 1000 });
    db.repos.snapshots.delete(parseSnapshotId('snap-1'));
    expect(db.repos.snapshots.list(parseSessionId('sess-1')).rows).toHaveLength(0);
  });

  it('getSnapshot returns undefined for unknown id', () => {
    expect(db.repos.snapshots.get(parseSnapshotId('nope'))).toBeUndefined();
  });

  it('getSnapshot returns snapshot data', () => {
    db.repos.sessions.upsert(SESSION);
    db.repos.snapshots.insert({ id: 'snap-1', sessionId: 'sess-1', label: '', trigger: 'auto', data: JSON.stringify(SNAP_DATA), createdAt: 1000 });
    const snap = db.repos.snapshots.get(parseSnapshotId('snap-1'));
  ...
  it('pruneSnapshots keeps only N newest auto snapshots per session (manual untouched)', () => {
    db.repos.sessions.upsert(SESSION);
    for (let i = 0; i < 7; i++) {
      db.repos.snapshots.insert({ id: `s${i}`, sessionId: 'sess-1', label: '', trigger: 'auto', data: '{}', createdAt: i });
    }
    db.repos.snapshots.insert({ id: 'manual-1', sessionId: 'sess-1', label: 'user save', trigger: 'manual', data: '{}', createdAt: 100 });
    db.repos.snapshots.prune(parseSessionId('sess-1'), 5);
    const snaps = db.repos.snapshots.list(parseSessionId('sess-1')).rows;
  ...
  it('pruneSnapshotsTiered deletes auto snapshots older than 30 days', () => {
    ...
    db.repos.snapshots.insert({ id: 'old', sessionId: 'sess-1', label: '', trigger: 'auto', data: '{}', createdAt: now - 35 * DAY });
    // 2 days old — should be kept
    db.repos.snapshots.insert({ id: 'recent', sessionId: 'sess-1', label: '', trigger: 'auto', data: '{}', createdAt: now - 2 * DAY });
    db.repos.snapshots.pruneTiered();
    const snaps = db.repos.snapshots.list(parseSessionId('sess-1')).rows;
    expect(snaps).toHaveLength(1);
  ...
  it('pruneSnapshotsTiered never deletes manual snapshots', () => {
    ...
    db.repos.snapshots.insert({ id: 'manual-old', sessionId: 'sess-1', label: 'user save', trigger: 'manual', data: '{}', createdAt: now - 60 * DAY });
    db.repos.snapshots.pruneTiered();
    const snaps = db.repos.snapshots.list(parseSessionId('sess-1')).rows;
    expect(snaps).toHaveLength(1);
  ...
  it('pruneSnapshotsTiered thins 1-7 day range to 1 per 3-hour window', () => {
    ...
    for (let i = 0; i < 6; i++) {
      db.repos.snapshots.insert({ id: `h${i}`, sessionId: 'sess-1', label: '', trigger: 'auto', data: '{}', createdAt: baseTime + i * 1000 });
    }
    db.repos.snapshots.pruneTiered();
    const snaps = db.repos.snapshots.list(parseSessionId('sess-1')).rows;
  ...
  it('pruneSnapshotsTiered keeps max 20 in last 24 hours', () => {
    ...
    db.repos.sessions.upsert(SESSION);
    for (let i = 0; i < 25; i++) {
      db.repos.snapshots.insert({ id: `r${i}`, sessionId: 'sess-1', label: '', trigger: 'auto', data: '{}', createdAt: now - i * 60_000 });
    }
    db.repos.snapshots.pruneTiered();
    const snaps = db.repos.snapshots.list(parseSessionId('sess-1')).rows;
