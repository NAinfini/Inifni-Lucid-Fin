...
Remove the `import fs from 'node:fs';` (line 3) — no longer needed.

**Step 2: Run tests**

Run: `npx vitest run apps/desktop-main/src/api-server`
Expected: PASS (or no test file — verify manually)
...
```bash
git add apps/desktop-main/src/api-server.ts
git commit -m "perf: convert api-server to async fs, cache pkg version"
```
